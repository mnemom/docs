...
AIP analyzes thinking blocks *after* the agent has already processed the injected content. The injection has occurred. AIP detects its effects on reasoning. By the time AIP raises a `boundary_violation`, the agent has already thought the problematic thoughts. In `enforce`, AIP prevents the resulting *action*, not the *reasoning* — the response is gated before delivery. In `observe` and `nudge`, AIP does not prevent the action at all; it records the verdict (and, for nudge, carries it into the agent's next turn).
...
**Do not read this limitation as "injected content is screened later."** Preventing injected content from reaching the model is the [Safe House](/concepts/safe-house) front door's job, and it happens on the request path, not on a later turn. That includes tool results: each one is screened in the same request that carries it back to the model, and a flagged tool result is withheld or decorated before the body is forwarded upstream. AIP sits behind that screen and catches what it scored below threshold. See [When the front door runs](/concepts/safe-house#when-the-front-door-runs).
...
AIP adds latency between agent turns. In the hosted gateway path the analysis LLM call typically completes in ~3-5 seconds, running in the background while the client already has its response (except in `enforce`, where the response itself is held until analysis returns). During this time, the agent is waiting only in `enforce` or when the SDK's `check()` is awaited directly.
...
**Implication:** AIP requires explicit architectural coordination to be preventive. In a gateway deployment under `enforce`, the gateway evaluates the response before delivering it, which adds latency; in `observe` and `nudge` the response is returned immediately and the verdict is recorded post-hoc. In an SDK deployment, the caller must await the signal before proceeding.
...
1. **Early warning.** Detection of misalignment *between* turns rather than *after* actions, reducing the window of vulnerability from 30-60 seconds (AAP's post-hoc observer) to a few seconds (AIP's gateway path).
...
5. **AAP integration.** Checkpoints link to AP-Traces, enriching the post-hoc audit trail with per-turn analysis data.
