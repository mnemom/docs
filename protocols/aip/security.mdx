...
**Status:** Normative for AIP SDK 1.3.0
...
**AIP detection:** The drift detection system tracks consecutive non-`clear` verdicts within the session. When that streak reaches the sustained-checks threshold (default 3), a drift alert is raised; its severity (`low` / `medium` / `high`) is derived from the window's `integrity_similarity` (clear-verdict ratio) against the 0.7 / 0.4 severity bands.
...
- Email: security@mnemom.ai (see [`/.well-known/security.txt`](https://mnemom.ai/.well-known/security.txt) for the current disclosure policy)
- Do NOT open a public GitHub issue for security vulnerabilities
- We acknowledge good-faith reports within 3 business days, confirm reproduction within 14 days, and commit to a fix or mitigation within 90 days of acknowledgment
