...
An Alignment Card declares what your agent is, what values it holds, and what it will and won't do autonomously. This is the unified / ADR-039 card shape (SDK v2.0.0) -- if you have a card from before mid-2026, see the [migration note](/protocols/aap/specification#4-10-migrating-from-the-legacy-0-5-0-card-shape) in the specification.
...
card = AlignmentCard(
    ...
    autonomy_mode="observe",   # off | observe | nudge | enforce
...
You can also generate a starter card from the CLI, which always emits the current unified shape:

```bash
aap init --values "principal_benefit,transparency,minimal_data" --output alignment-card.json
```
...
print(f"Recommended action: {result.recommended_action}")  # "proceed" | "review" | "deny"
...
#   Trace IDs: ['trace-12', 'trace-13', 'trace-14', 'trace-15']
```
...
```python
from aap import (
    AlignmentCard, Principal, Values, Autonomy, Audit,
...
card = AlignmentCard(
    ...
    integrity_mode="observe",
    principal=Principal(type="human", identifier="demo-agent-owner", relationship="delegated_authority"),
...
trace = APTrace(
    ...
    escalation=Escalation(
        evaluated=True, required=False, reason="Within autonomy envelope"
    ),
)
...
- **[Interactive Playground](https://mnemom.github.io/aap/playground/)** -- Try verification in your browser with SSM visualization, no server required (runs client-side via WebAssembly)
