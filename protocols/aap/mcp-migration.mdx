...
SERVER_ALIGNMENT = AlignmentCard(
    ...
    principal={
        "type": "human",
        "identifier": "mcp-filesystem-owner",
...
`Action.type` is one of `recommend | execute | escalate | deny` (there is no `tool_invocation` value), and `Action.category` is one of `bounded | escalation_trigger | forbidden` (not `"escalate"`). `Decision.alternatives_considered` requires at least one entry, and `selected` MUST be one of those entries' `option_id` -- it cannot be `None` or reference an empty list. `Decision.values_applied` must be a plain list of strings, so read `SERVER_ALIGNMENT.values.declared` (an attribute on the `Values` model), not `SERVER_ALIGNMENT.values["declared"]`:
...
# Maps the decorator's simple category to the real ActionCategory enum
_CATEGORY_MAP = {
    "bounded": "bounded",
    "escalate": "escalation_trigger",
    "forbidden": "forbidden",
}
...
def traced_tool(tool_name: str, category: str):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            action_category = _CATEGORY_MAP[category]
            escalation_required = category == "escalate"

            if category == "forbidden":
                # Log the attempt but don't execute. alternatives_considered
                # still needs >= 1 entry, and selected must reference it.
                trace = APTrace(
                    trace_id=trace_id,
                    agent_id="mcp-filesystem",
                    ...
                    decision=Decision(
                        alternatives_considered=[
                            {"option_id": "deny", "description": f"Deny {tool_name}"},
                        ],
                        selected="deny",
...
class AlignedMCPClient:
    ...
    async def invoke_tool(self, tool_name: str, arguments: dict):
        ...
        server_envelope = self.server_alignment.get("autonomy") or self.server_alignment.get("autonomy_envelope", {})
...
SERVER_ALIGNMENT = AlignmentCard(
    ...
    agent_id="mcp-filesystem",
    issued_at="2026-01-31T12:00:00Z",
    principal={"type": "human", "identifier": "mcp-filesystem-owner", "relationship": "delegated_authority"},
    values={
...
def read_file(path: str) -> str:
    ...
    try:
        ...
        trace = APTrace(
            ...
            card_id=SERVER_ALIGNMENT.card_id,
            timestamp=timestamp,
            action=Action(type="execute", name="read_file", category="bounded"),
            decision=Decision(
            ...
            context={"metadata": {"path": path, "bytes_read": len(content)}},
        )
        store_trace(trace)
        return content

    except Exception as e:
        # Trace the failure. alternatives_considered still needs >= 1 entry,
        # and selected must reference one of them -- neither can be empty/None.
        trace = APTrace(
            trace_id=trace_id,
            agent_id="mcp-filesystem",
            card_id=SERVER_ALIGNMENT.card_id,
            timestamp=timestamp,
            action=Action(type="deny", name="read_file", category="bounded"),
            decision=Decision(
                alternatives_considered=[{"option_id": "failed", "description": "Read attempt failed"}],
                selected="failed",
...
def write_file(path: str, content: str, approved: bool = False) -> dict:
    ...
    if not approved:
        trace = APTrace(
            ...
            card_id=SERVER_ALIGNMENT.card_id,
            timestamp=timestamp,
            action=Action(type="escalate", name="write_file", category="escalation_trigger"),
            decision=Decision(
                alternatives_considered=[{"option_id": "blocked", "description": "Write blocked pending approval"}],
                selected="blocked",
    ...
    trace = APTrace(
        ...
        card_id=SERVER_ALIGNMENT.card_id,
        timestamp=timestamp,
        action=Action(type="execute", name="write_file", category="escalation_trigger"),
        decision=Decision(
        ...
        escalation=Escalation(
            ...
            triggers_checked=[{"trigger": "tool == 'write_file'", "matched": True}],
            required=True,
            # principal_response is a PrincipalResponse object, not a bare string
            principal_response={"decision": "approved", "timestamp": timestamp},
            reason="Write operations require approval",
        ),
        context={"metadata": {"path": path, "bytes_written": bytes_written}},
    )
    store_trace(trace)
...
def delete_file(path: str) -> dict:
    ...
    trace = APTrace(
        ...
        card_id=SERVER_ALIGNMENT.card_id,
        timestamp=timestamp,
        action=Action(type="deny", name="delete_file", category="forbidden"),
        decision=Decision(
            alternatives_considered=[{"option_id": "deny", "description": "Delete blocked"}],
            selected="deny",
            selection_reasoning="Forbidden action blocked",
