---
title: "Mnemom Trust Score™ Methodology"
description: "Normative specification for the Mnemom Trust Score™ algorithm — composite formula, component definitions, confidence levels, and anti-gaming measures."
sidebarTitle: "Scoring Methodology"
icon: "calculator"
---

# Mnemom Trust Score™ Methodology
...
| Violation Recency | `violation_recency` | 0.20 |
...
**Weight rationale**: Integrity Ratio receives the highest weight (40%) because it directly measures the agent's core behavioral alignment. Violation Recency and Drift Stability share 20% each because they measure temporal patterns that are strong predictors of future behavior. Trace Completeness and Coherence Compatibility receive 10% each as supporting signals.
...
### 2.2 Violation Recency (w = 0.20)

Penalizes recent violations more heavily than older ones using exponential decay.
...
Where `max_recent_impact` is the maximum impact value across all violations within the scoring window:

```
impact(violation) = severity_weight × 2^(-age_weeks / half_life)
```

**Parameters:**
- `half_life`: 1 week (7 days). A violation's impact halves every week.
- `severity_weight`:
  - `boundary_violation`: 1.0
  - `review_needed` with `severity = "high"` or `"critical"`: 0.5
  - Other `review_needed`: 0.2
- `age_weeks`: `(now - violation_timestamp) / (7 × 24 × 60 × 60 × 1000)` in fractional weeks

**Decay schedule:**

| Age | Impact (boundary_violation) |
|-----|---------------------------|
| Today | 1.000 |
| 1 week | 0.500 |
| 2 weeks | 0.250 |
...
- If the most recent violation is > 8 weeks old: effective impact < 0.004, score rounds to 1000
- Multiple simultaneous violations: only the maximum impact is used (not cumulative)

**Data source:** AIP checkpoints with `verdict != "clear"`, filtered by timestamp.
...
**Caching:** The API caches computed scores with a 5-minute TTL. Consumers MAY receive scores up to 5 minutes stale. Badges have a separate 60-minute cache.
...
### 8.4 Temporal Bias in Violation Recency

The 1-week half-life means violations are effectively forgiven after 8 weeks. An agent with a history of serious violations that occurred 3 months ago will show a perfect Violation Recency score. Historical snapshots (available via the history endpoint) provide the full longitudinal record.
