...
A2A and AAP are complementary, independently-governed protocols that answer different questions in the agentic AI stack:

```
+---------------+---------------+---------------------------+
|      MCP      |      A2A      |         AAP / AIP         |
|  Agent<->Tools|  Agent<->Agent|   Alignment & Integrity   |
+---------------+---------------+---------------------------+
| "What tools   | "What can we  | "Should we work together, |
|  can I use?"  |  do together?"|  and can we prove it?"    |
+---------------+---------------+---------------------------+
```

MCP connects agents to tools. A2A connects agents to each other. AAP verifies that coordinating agents share compatible values and produces auditable decision trails. AIP adds real-time integrity monitoring of agent reasoning. AAP is designed to extend A2A Agent Cards (this guide) and MCP tool manifests ([MCP migration guide](/protocols/aap/mcp-migration)) rather than compete with either.
...
| `skills` | `autonomy.bounded_actions` | Which skills are safe to execute autonomously |
| `capabilities` | `autonomy` | Escalation triggers, forbidden actions, spending limits |
...
| *(no equivalent)* | `audit` | Trace format, retention, queryability guarantees |
...
|  +---------------------------------------------+ |
|  |          AAP Alignment Block                | |
|  |  principal, values, autonomy,               | |
|  |  audit, autonomy_mode, integrity_mode,      | |
|  |  extensions                                  | |
|  +---------------------------------------------+ |
+-------------------------------------------------+
```
...
```json
{
  ...
  "alignment": {
    ...
    "principal": {
      "type": "human",
      "identifier": "shopping-assistant-owner",
...
def delegate_task(my_card: dict, their_agent_card: dict, task: dict):
    ...
    if result.compatible:
        # Values are compatible (zero conflicts, score >= 0.70), proceed.
        # Note: result.proceed always mirrors result.compatible -- it is
        # not an independent "minor vs. major conflict" signal.
        return execute_delegation(their_agent_card, task)

    # Not compatible: either a direct conflict, or the score fell short
    # of MIN_COHERENCE_FOR_PROCEED. Surface what was found and escalate.
...
function delegateTask(myCard: A2AAgentCard, theirCard: A2AAgentCard, task: unknown) {
  ...
  // result.proceed always mirrors result.compatible -- checking one is
  // checking the other, not an independent "minor vs. major" signal.
  if (result.compatible) {
...
def search_products_with_trace(card_id: str, query: str, preferences: dict):
    ...
    trace = APTrace(
        ...
        action=Action(
            type="recommend",  # ActionType has no "search" value
...
def coherence_check():
    ...
    return jsonify({
        ...
        "conflicts": [
            {"description": c.description, "conflict_type": c.conflict_type}
...
USER_AGENT_CARD = {
    ...
    "alignment": {
        ...
        "issued_at": "2026-01-31T12:00:00Z",
        "principal": {"type": "human", "identifier": "user-shopping-agent-owner", "relationship": "delegated_authority"},
        "values": {
            "declared": ["principal_benefit", "transparency", "minimal_data"],
        ...
        "audit": {"trace_format": "ap-trace-v1", "retention_days": 30, "queryable": True},
    }
}
...
VENDOR_AGENT_CARD = {
    ...
    "alignment": {
        ...
        "issued_at": "2026-01-31T12:00:00Z",
        "principal": {"type": "organization", "identifier": "vendor-deals-agent-org", "relationship": "delegated_authority"},
        "values": {
        ...
        "audit": {"trace_format": "ap-trace-v1", "retention_days": 90, "queryable": True},
    }
}
...
def coordinate_with_vendor():
    ...
    if result.proceed:
        print("Proceeding with coordination (compatible: zero conflicts, score >= 0.70)")
    else:
...
```python
from aap import EU_COMPLIANCE_AUDIT, EU_COMPLIANCE_VALUES

alignment = AlignmentCard(
    # ...
    values=EU_COMPLIANCE_VALUES,
    audit=EU_COMPLIANCE_AUDIT,  # 90-day retention, queryable (per compliance.py)
)
```
...
```typescript
import { EU_COMPLIANCE_AUDIT, EU_COMPLIANCE_VALUES } from '@mnemom/agent-alignment-protocol';

const alignment = {
  // ...
  values: EU_COMPLIANCE_VALUES,
  audit: EU_COMPLIANCE_AUDIT, // 90-day retention, queryable
};
```
...
Beyond value coherence, agents can advertise their [Mnemom Trust Rating](/concepts/reputation-scores) directly in A2A Agent Cards. This enables programmatic trust decisions before delegation -- an agent can refuse to work with peers below a certain trust rating threshold.
