...
```
AlignmentCard  (unified / ADR-039 shape, SDK 2.0.0)
+-- card_version: str ("unified/2026-04-26")
+-- card_id: str (unique identifier)
+-- agent_id: str (agent DID or identifier)
+-- issued_at: datetime
+-- expires_at: datetime (optional)
+-- autonomy_mode: AlignmentMode (off | observe | nudge | enforce)
+-- integrity_mode: AlignmentMode (off | observe | nudge | enforce)
+-- principal: Principal
|   +-- type: PrincipalType (human | organization | agent | unspecified)
|   +-- identifier: str (required unless type == "unspecified")
|   +-- relationship: RelationshipType (delegated_authority | advisory | autonomous)
...
|   +-- hierarchy: str (optional, "lexicographic" | "weighted" | "contextual")
|   +-- definitions: dict[str, ValueDefinition] (optional, custom values)
+-- autonomy: Autonomy   (renamed from legacy `autonomy_envelope`)
...
+-- audit: Audit   (renamed from legacy `audit_commitment`; `storage` sub-object removed)
|   +-- retention_days: int
|   +-- queryable: bool
|   +-- tamper_evidence: str (optional, "append_only" | "signed" | "merkle")
...
+-- decision: Decision
|   +-- alternatives_considered: list[Alternative]  (minimum 1)
|   |   +-- option_id: str
|   |   +-- description: str
|   |   +-- score: float (0.0-1.0, optional)
...
|   +-- value_scores: dict[str, ValueScore] (optional, SDK >= 1.3.0)
...
+-- context: TraceContext (optional)
    ...
    +-- conversation_turn: int (optional)
    +-- prior_trace_ids: list[str] (optional)
...
Performs seven verification checks (SPEC Section 7.3), tracked in `verification_metadata.checks_performed`:
...
7. behavioral_similarity -> Is the trace semantically similar to the card (SSM analysis)?
```

Returns `VerificationResult`:
- `verified: bool` -- True if no violations
- `recommended_action: "proceed" | "review" | "deny"` -- Derived from violations/warnings
- `violations: list[Violation]` -- Type, description, severity
- `warnings: list[Warning]` -- Near-boundary and low-similarity conditions
...
where conflict_penalty = min(1.0, 0.5 * (conflicts / required))
```

Returns `CoherenceResult`:
- `compatible: bool` -- Zero conflicts AND score >= 0.70 (`MIN_COHERENCE_FOR_PROCEED`)
- `score: float` -- Coherence score [0, 1]
- `value_alignment` -- Matched, unmatched, conflicts
- `proceed: bool` -- Mirrors `compatible`
- `proposed_resolution` -- If incompatible, suggests escalation

#### `detect_drift(card, traces, thresholds) -> list[DriftAlert]`

Analyzes trace sequence for behavioral drift (SPEC Section 8, v1.2.0 methodology):

```
1. Extract structural features per trace (action, values applied, escalation, confidence)
2. Compute a baseline centroid from the first N traces
3. For each subsequent trace, compare to the baseline centroid via cosine similarity
4. Track low-similarity streak
5. If streak >= sustained_threshold:
...
Structural feature extraction for drift detection (not TF-IDF -- see [calibration](/protocols/aap/calibration) for why content tokens are excluded from drift scoring). TF-IDF weighting is used separately, for text-to-text SSM similarity:

```python
class FeatureExtractor:
    def extract_card_features(card: dict) -> dict[str, float]:
        # Returns structural feature vector from declared values, bounded/forbidden actions

    def extract_trace_features(trace: dict) -> dict[str, float]:
        # Returns structural feature vector from applied values, action, escalation, confidence
...
DEFAULT_SUSTAINED_TURNS_THRESHOLD = 3    # Alert after N consecutive low traces (Python name;
                                          # TypeScript SDK: DEFAULT_SUSTAINED_CHECKS_THRESHOLD)
NEAR_BOUNDARY_THRESHOLD = 0.35           # Warn when confidence/score below
CONFLICT_PENALTY_MULTIPLIER = 0.50       # Coherence penalty per conflict
MIN_COHERENCE_FOR_PROCEED = 0.70         # Minimum score to proceed
```
...
- **Live**: [mnemom.github.io/aap/playground](https://mnemom.github.io/aap/playground/)
- **Runtime**: Pyodide (Python in WASM)
- **API**: `window.AAP.verifyTrace()`, `window.AAP.checkCoherence()`, `window.AAP.detectDrift()`
