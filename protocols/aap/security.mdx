...
3. Cryptographic requirements for implementers who add them
...
<Note>
**What the reference SDKs actually do today.** `verify_trace()` in both the Python and TypeScript SDKs performs **structural verification only** -- it checks a trace against a card's declared fields. It does not sign, hash-chain, or otherwise cryptographically attest cards or traces. Everything in this document phrased as "cards SHOULD be signed," "traces SHOULD be signed," etc. is a recommendation for implementers layering security on top of AAP, not a description of built-in SDK behavior. If you need an out-of-the-box cryptographic integrity guarantee over an agent's actual reasoning, pair AAP with [AIP](/protocols/aip/specification), whose checkpoints are signed, hash-chained, and Merkle-attested by the hosted platform.
</Note>
...
4. Cards MUST include:
   ...
   - `signature`: Base64-encoded Ed25519 signature over canonical JSON (OPTIONAL -- the SDK does not require it)

> **Note:** The SDK focuses on verification logic. Cryptographic signing is a recommended production enhancement. See [specification](/protocols/aap/specification) Section 9.2 for alignment.
...
> **Note:** The SDK does not enforce `sequence_number`. Gap detection is a recommended production enhancement for high-assurance deployments.
...
This example layers a `nonce` and mandatory `signature` onto the base `value_coherence_check` message (Section 6.3.3 of the [specification](/protocols/aap/specification#6-value-coherence-handshake)) as a security hardening recommendation -- neither field is part of the base schema today:
...
The card schema does not pin a signature algorithm -- `value_coherence.schema.json`'s optional `signature` block carries an `algorithm` field (Section 6, `"Ed25519"` in the reference examples) precisely so implementations can name whichever algorithm they actually used. If you introduce a new algorithm, verifiers MUST reject a signature whose declared algorithm they don't recognize rather than assume Ed25519.
