...
description: "Full protocol specification for the Agent Alignment Protocol (AAP) -- the unified / ADR-039 card shape shipped in SDK v2.0.0"
sidebarTitle: "Specification"
icon: "book"
---

**Card shape**: unified / ADR-039 (`card_version: "unified/2026-04-26"`)
...
document uses the current unified shape. See the [migration note](#4-10-migrating-from-the-legacy-0-5-0-card-shape) if you have cards issued before 2026-06.
</Note>
...
An Alignment Card is a structured document declaring an agent's alignment posture. It MUST be machine-readable (JSON) and SHOULD be human-readable. This is the **unified / ADR-039** card shape -- the same shape accepted by `mnemom card validate` and the Mnemom platform's `PUT /v1/agents/{agent_id}/alignment-card` endpoint, so an agent can serve one card that satisfies both AAP and the platform.
...
See [`schemas/alignment-card.schema.json`](https://github.com/mnemom/aap/blob/main/schemas/alignment-card.schema.json) for the complete, canonical unified / ADR-039 JSON Schema.
...
This page describes the card shape and verification behavior shipped in **SDK 2.0.0** (`agent-alignment-protocol` on PyPI, `@mnemom/agent-alignment-protocol` on npm). The headline change from 1.x is the migration to the unified / ADR-039 card shape (Section 4.10). Other notable behavior changes carried by 2.0.0 and 2.0.1:
...
- **2.0.0** -- Card shape migrated to unified / ADR-039 (Section 4). `AutonomyEnvelope`/`AuditCommitment` SDK types renamed to `Autonomy`/`Audit`; `AuditStorage`/`StorageType` removed; `AlignmentMode` (and the `autonomy_mode`/`integrity_mode` card fields) added.
