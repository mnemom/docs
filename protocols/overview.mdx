...
Mnemom is built on two complementary open-source verification protocols (AAP and AIP) plus a governance layer (CLPI). Together, they provide retrospective verification, per-turn integrity assurance, and active policy enforcement for AI agent behavior.
...
| **When it checks** | After the agent acts (post-hoc) | While the agent thinks (per turn; enforce gates same-turn, observe/nudge post-hoc) |
...
**AIP** is per-turn assurance. During execution, AIP analyzes the LLM's thinking blocks (Anthropic), thought parts (Gemini), or reasoning summaries (OpenAI) to detect active threats: prompt injection attempts, gradual value drift within a session, or manipulation by adversarial inputs. In `enforce`, AIP gates the resulting response before it is delivered — catching problems before the agent acts on compromised reasoning; in `observe` and `nudge` the verdict is recorded post-hoc (nudge carries it into the agent's next turn).

AIP's per-turn cadence describes the *reasoning* pass only. It is not the schedule on which inbound content is screened, and injected content does not wait for AIP to see it: [Safe House](/concepts/safe-house) screens each inbound surface a request carries — including the tool results in that request's own body — at the front door, before the request is forwarded. AIP sits behind that screen as defense in depth. See [When the front door runs](/concepts/safe-house#when-the-front-door-runs).
...
| Integrity assurance | **AIP** | Per-turn analysis of reasoning for active threats |
...
<CardGroup cols={2}>
  ...
  <Card title="Integrity Checkpoints" icon="shield-check" href="/concepts/integrity-checkpoints">
    Per-turn AIP analysis results. Each checkpoint contains a verdict (`clear`, `review_needed`, `boundary_violation`), identified concerns, and confidence levels.
  </Card>
...
## Protocol versions in production

The wire-format protocol versions above are stable; the table below is about the concrete runtime and SDK builds that implement them today.

| Component | Version | Role |
|---|---|---|
| Alignment card contract | unified card shape (`card_version: unified/2026-04-26`) | What `api.mnemom.ai` accepts on card writes. Legacy `autonomy_envelope` / `audit_commitment` locations are rejected. |
| AAP SDKs (TypeScript + Python) | 2.0.0+ | Emit the unified card shape above. |
| Hosted gateway integrity checks | AIP 1.3.0 | Per-turn `enforce`/`observe`/`nudge` verdicts, plus `@mnemom/aip-otel-exporter` 0.13.0 for telemetry export. |
| Hosted observer verification | AAP 1.3.1 | Verifies AP-Traces against cards using the 1.3.x behavioral-similarity feature extractor. |

If you consume verification scores from the hosted service, they follow the 1.3.x extractor. AAP 2.0.1 changed that extractor's feature set, and similarity scores are not numerically comparable across the 1.3.x / 2.0.1 boundary — treat a self-hosted upgrade across that line as a score reset point, not a continuation of the same series.

For direct or self-hosted use, the current SDK releases are `@mnemom/agent-alignment-protocol` / `agent-alignment-protocol` 2.0.0 and `@mnemom/agent-integrity-protocol` 1.3.0 / `agent-integrity-proto` 1.3.0 -- see [SDK packages](#sdk-packages) above.
