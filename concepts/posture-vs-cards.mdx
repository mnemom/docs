...
**No.** Postures and cards are *parallel* artifact types. They share infrastructure (the composer idiom, read-through caching, the audit log, strictest-wins rules) but their data planes are independent. A posture never folds into a card; a card never folds into a posture.
...
| Axis | Cards | Posture |
|---|---|---|
| **Time** | Synchronous, same-turn — and per *surface*, not per turn: the front door fires once for the inbound message and again for each tool result the same request carries | Asynchronous, periodic sweep, cross-turn carryover |
