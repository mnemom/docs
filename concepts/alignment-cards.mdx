...
When you connect an agent via smoltbot, a default alignment card is created automatically. This default card uses generic values and minimal autonomy -- it is a starting point, not a finished card. To make the card meaningful, you should customize it to reflect your agent's actual values, boundaries, and capabilities.
...
| Method | Best For |
|---|---|
| **CLI** (`smoltbot card publish`) | Local development, CI/CD pipelines, version-controlled card files |
| **Dashboard** (agent page card editor) | Quick edits, visual exploration, teams that prefer a GUI |
| **API** (`PATCH /v1/agents/:id/card`) | Programmatic updates, automation, custom tooling |
...
Use `smoltbot card validate` for local pre-flight checks before publishing. This runs the same validation locally without making any API calls, and is CI-friendly (exit code 0 on pass, 1 on fail).

See the [CLI reference](/smoltbot/cli) for full command documentation.
...
Organizations can define a base card template that all agents inherit. Agent cards layer on top of the org template -- org values cannot be removed by individual agents, ensuring consistent organizational alignment policy.

This is similar to how [custom conscience values](/smoltbot/conscience-values) work at the org level, but applied to the full alignment card structure rather than just the conscience prompt.

See the [Organization Card Templates guide](/smoltbot/org-card-templates) for setup and configuration details.
