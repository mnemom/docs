...
<Note>
**This page describes the AAP protocol-level card structure** — the minimum surface every AAP implementation must support (the **unified card shape**, shipped in AAP SDK 2.0.0 and later). In production, Mnemom stores and serves a richer version of this same shape that adds first-class `conscience`, `capabilities`, `enforcement`, and `integrity_config` sections, and composes across up to four scopes (platform > org > team *(optional)* > agent).

For the full production model, see [Agent Cards](/concepts/agent-cards) (the two-card product story), [Card Composition](/concepts/card-composition) (scope hierarchy), and the normative [Alignment Card Schema](/specifications/alignment-card-schema).
</Note>

<Note>
  Alignment Cards declare intent, not guarantee behavior. An agent can publish a card claiming any set of values. The card becomes meaningful only when paired with AP-Traces that can be verified against it and integrity checkpoints that analyze the agent's reasoning per turn.
</Note>
...
An Alignment Card contains six required blocks and one optional block:

| Block | Purpose | Required |
|---|---|---|
| **Identity** | Agent ID, card ID, version, timestamps | Yes |
| **Mode switches** | `autonomy_mode` / `integrity_mode` -- master switches for the action-policing and conscience pipelines | Yes |
| **Principal** | Who the agent serves and how | Yes |
| **Values** | What the agent prioritizes | Yes |
| **Autonomy** | What the agent can do independently | Yes |
| **Audit** | How the agent logs decisions | Yes |
| **Extensions** | Protocol-specific additions (A2A, MCP) | No |

### Identity fields

Every card begins with identity and versioning metadata, plus the two mode switches:
...
- `card_version` is the date-anchored schema version (pattern `^unified/\d{4}-\d{2}-\d{2}$`).
...
- `autonomy_mode` and `integrity_mode` are the master switches (`off` / `observe` / `nudge` / `enforce`) for, respectively, the AAP action-policing checks and the [AIP](/concepts/integrity-checkpoints) per-turn conscience pipeline that shares this card.
...
| `read_documents` (a capability) | `autonomy.bounded_actions` |
...
The `autonomy` block defines the boundaries of independent action:
...
- **forbidden_actions**: The agent's self-declared behavioral commitment — actions it promises never to take. These compose via deny-overrides union with platform and org floors; the agent can add entries but cannot remove a higher-scope entry.
...
The `audit` block declares how the agent logs and exposes its decisions:
...
Any org owner or admin can define a base alignment card template (`PUT /v1/alignment/org/{org_id}`) that all of the org's agents inherit — this is a core part of the composition model, not a gated add-on. Agent cards layer on top of the org template; org values cannot be removed by individual agents, ensuring consistent organizational alignment policy.
...
See the [Organization Card Templates guide](/gateway/org-card-templates) for setup details, and [Card Composition](/concepts/card-composition) for the full composition model.
...
- [Integrity Checkpoints](/concepts/integrity-checkpoints) -- Per-turn analysis of agent thinking against cards
