...
| **Update frequency** | Every 6 hours | Every 6 hours + on-demand |
...
## Cryptographic verification for teams

Team reputation scores have their own verification endpoint, public and requiring no authentication:

```
GET /v1/teams/{team_id}/reputation/verify
```

It returns the team's `score` and `grade` alongside a `verification` block with a `proof_hash`, the `computed_at` timestamp, and `total_assessments` — enough for a third party to confirm the returned score matches what Mnemom last computed for that team.

<Note>
This is a hash-based verification bundle, not a zero-knowledge proof. For ZK-proven risk computations (a different, related surface), see [Risk Assessment](/concepts/risk-assessment#zero-knowledge-proofs).
</Note>
...
The `@mnemom/reputation` client exposes a `getTeamA2AReputationExtension()` helper and the shared type library defines an `A2ATeamTrustExtension` shape, mirroring the [individual A2A trust extension](/concepts/reputation-scores#a2a-trust-extension). As of this writing the team reputation endpoint does not yet populate an `a2a_trust_extension` field on its response, so calling the helper against a live team throws rather than returning a trust block. Use the [plain team reputation response](#api-reference) (`score`, `grade`, `confidence`) for A2A trust signaling until this ships.
...
Requires a Bearer token or API key. Returns the full score with all components for teams the caller can access.
...
The public [directory search](/api-reference/team-overview) and compare endpoints additionally enrich each row with `name`, `avatar_url`, `member_count`, and `assessment_count` -- those fields are not present on the plain per-team GET above.
...
```typescript
import { getTeamReputation, createTeamReputationGate } from '@mnemom/reputation';

// Get current team reputation
const reputation = await getTeamReputation('team-abc123');
...
// Gate a decision on a minimum team score or grade
const gate = createTeamReputationGate({ minScore: 700, minGrade: 'A' });
const result = await gate.check('team-abc123');
if (!result.allowed) {
  console.log('Denied:', result.reason);
}

// Weekly history has no dedicated SDK helper -- call the endpoint directly
const history = await fetch('https://api.mnemom.ai/v1/teams/team-abc123/reputation/history', {
...
# Get current team reputation (requires auth)
response = httpx.get(f"{API_BASE}/v1/teams/team-abc123/reputation", headers=headers)
...
# Get weekly history
history = httpx.get(f"{API_BASE}/v1/teams/team-abc123/reputation/history", headers=headers)
