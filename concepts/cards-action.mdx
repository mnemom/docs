...
The action is **read-only**. It never writes a canonical card. The merge step doesn't auto-publish — an operator runs [`publish_manifest`](/guides/publishing-manifests) to update the canonical card after merge.
...
- **Explain** — the policy engine's current trace + suggested remediations (with HTTP verb + URL pointing at [W3.1 sub-resource verbs](/concepts/sub-resource-verbs)).
...
- [Care Framing Doctrine](/concepts/care-framing-doctrine) — the forbidden-words list the action enforces in every comment.
- [Publishing Manifests](/guides/publishing-manifests) — the operator-PUT path that runs after merge.
