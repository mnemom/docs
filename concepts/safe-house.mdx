...
Safe House uses the canonical four-mode enum shared with the [Protection Card](/concepts/protection-card) and the [Alignment Card](/concepts/alignment-cards) master switches per ADR-039. Same words, same semantics, same UI picker.
...
<Note>
**Vocabulary canonicalization (ADR-039).** Earlier releases of this page documented `disabled / simulate / observe / enforce`. The canonical post-ADR-039 vocabulary is `off / observe / nudge / enforce` — the same enum used by the [Protection Card](/concepts/protection-card), the Alignment Card master switches, and the cross-tenant AEGIS [Managed Rules](/concepts/managed-rules). The legacy `simulate` mode has been merged into `observe` (full evaluation, no blocking); `disabled` has been renamed `off`. The new mode is `nudge` (advisory injection without blocking) between `observe` and `enforce`.
</Note>
...
Trust is **binary** — a match means "skip detection." Earlier designs that exposed a graduated `risk_multiplier` were withdrawn during the ADR-037 schema unification; tuning sensitivity is the job of `thresholds.{warn, quarantine, block}`, not per-source weights.
