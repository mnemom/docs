...
description: "Per-turn analysis of LLM reasoning to detect prompt injection, manipulation, and drift"
sidebarTitle: "Integrity Checkpoints"
icon: "shield-check"
---

{/* concept-fields: bounded_actions, linked_trace_id, thinking_block_hash, boundary_violation, review_needed */}
...
Integrity Checkpoints are the primary data structure of the [Agent Integrity Protocol (AIP)](/protocols/aip/specification), the sister protocol to [AAP](/protocols/aap/specification). See [Protocol versions in production](/protocols/overview#protocol-versions-in-production) for which AIP runtime version the hosted gateway runs.
...
<Note>
**Integrity checkpoints are not where inbound content gets screened.** AIP reads the model's reasoning — it runs on the response path, after the model has already read its input. Screening the input, including each tool result travelling back to the model, is the Safe House front door's job, and it happens on the request path in the same request that carries the content. Where the examples below show AIP catching injection "embedded in tool output," that is AIP catching a payload the front door scored below threshold — defense in depth behind the front door, not the tool-result screen itself. See [When the front door runs](/concepts/safe-house#when-the-front-door-runs).
</Note>
...
| `output_misalignment` | Output contradicts declared card values even though the thinking block shows no adversarial intent | medium - critical |
| `output_injection_compliance` | Output follows injected instructions that the thinking block correctly identified and rejected — the agent "knew better" but acted contrary to its own reasoning | high - critical |

A concern at `critical` severity triggers a `boundary_violation` verdict, as does `high` severity in `prompt_injection`, `deceptive_reasoning`, or `output_injection_compliance`.
...
| **OpenAI** | `reasoning_content` field | 0.9 | Explicit reasoning field, if present |
...
<Warning>
**OpenAI's `reasoning_content` field is not populated through Mnemom's gateway today.** OpenAI's Chat Completions API — the shape the gateway proxies, streaming or not — never returns reasoning content, for any OpenAI model. The adapter above extracts it when present (for a self-hosted integration that reads it directly from a provider that does surface it), but on Mnemom's managed and self-hosted gateway, every OpenAI request extracts an empty thinking block and falls through to the minimum-evidence skip below — no analysis-LLM call, no verdict beyond the synthetic `clear`. See [Provider Support](/concepts/provider-support) for the full accounting.
</Warning>
...
A complete integrity check from response to signal. The `card` shape below is the AIP SDK's own lightweight `AlignmentCard` type (`values` is an array of `{name, priority}` objects, lower number = higher priority) -- see [AIP quickstart](/protocols/aip/quickstart#2-define-an-alignment-card) for how it differs from the full AAP unified card:
...
card = {
    "card_id": "ac-my-card",
    "values": [
        {"name": "principal_benefit", "priority": 1},
        {"name": "transparency", "priority": 2},
...
|---|---|---|
| **Timing** | Per-turn (enforce gates same-turn; observe/nudge post-hoc) | Post-hoc, after action |
