...
{/* concept-fields: conflicts_with, timestamp */}
...
Compatibility requires **both** conditions: the score must clear `0.70` (`MIN_COHERENCE_FOR_PROCEED`) *and* there must be zero explicit conflicts. A pair that scores above 0.70 but has even one explicit conflict is still `compatible: false` — the conflict, not the score, is the harder gate. Below the threshold, or with any conflict present, the recommendation is to negotiate, modify scope, or escalate to principals.
...
- [Integrity Checkpoints](/concepts/integrity-checkpoints) -- Per-turn monitoring during collaboration
