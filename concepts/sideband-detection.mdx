...
<Note>
**"Next turn" on this page means the cron half only.** Nothing in the gateway's front door is deferred to a later turn: the inbound message is screened at request arrival, and tool results are screened [turn-internally](/concepts/safe-house#turn-internal-tool-result-screening) — inline, one result at a time, with a flagged result withheld from the same request. Sideband advisories are a separate, team-shaped mechanism; they are not a re-screen of anything the front door saw.
</Note>
...
Runtime checkpoints intervene same-turn under `enforce`; sideband checkpoints intervene cross-turn via the advisory carryover.
