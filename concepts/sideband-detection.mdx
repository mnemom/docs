...
**Sideband detection** is the asynchronous half of Mnemom's Safe House. While the gateway intervenes [same-turn at four checkpoints](/concepts/safe-house) on each agent request, the observer runs a parallel cron sweep across teams of agents and emits *advisories* the agent receives at the start of its next turn.
