...
Per ADR-044 (amended 2026-05-03): there is **no auto-created default team**. Teams are an explicit org-admin construct. If a customer-shaped need for "agents auto-land in a specific team" arises, that lands as an opt-in `default_team_id` setting on the org — additive, not breaking.
...
The save button is gated on **org owner / org admin** role today. (Team Admin role lands in a later milestone — see [Team Admin role](./team-admin-role) when published.)
...
- **Team scope is not a recipe.** [Recipes](./recipes-overview) are an orthogonal data plane (ADR-041) for detection rules. They will gain team scope on the same rationale as cards (cataloged as a follow-on); until then, recipes are platform/org/agent-scope only.
