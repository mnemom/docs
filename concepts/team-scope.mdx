...
- **Team identity cards** *(see [Team Reputation](/concepts/team-reputation))* — a single representative card auto-derived from the team's member cards via `POST /v1/teams/:id/card/derive`. Stored in `alignment_cards.team_id`. Used for fleet-coherence analysis and team-reputation scoring; not applied to member agents' cascades.
...
The save button is available to **org owner / org admin**, or to a user holding a [Team Admin](/concepts/team-admin-role) grant on that specific team.
...
Authorization: writes require org owner/admin **or** an active [Team Admin](/concepts/team-admin-role) grant on that team; full member roster (owner / admin / member / viewer / auditor) can read.
...
- **Team scope is not user-scope.** Users do not "belong to" a team in the composition sense — team membership (which agents inherit a team's templates) and the [Team Admin](/concepts/team-admin-role) role (which *user* may manage a team's templates/posture/roster) are two separate, orthogonal concepts.
...
- [Card composition](/concepts/card-composition) — the layered cascade overview
- [Personal Organization](/concepts/personal-organization) — why solo users still get an org context
- [Alignment Card](/concepts/alignment-cards) — the artifact at the agent leaf
- [Protection Card](/concepts/protection-card) — host-bound per-agent treatment policy
- [Team Reputation](/concepts/team-reputation) — team identity cards (separate, orthogonal feature)
