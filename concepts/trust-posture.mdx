...
A posture is a first-class, library-cataloged, versioned object, the same shape as a policy-management object in other security platforms: you assign it to teams, clone it to customize, and roll back when needed.
...
The two artifact types are parallel — they share infrastructure (composer, read-through cache, audit log) but their data planes are independent. A posture never folds into a card; a card never folds into a posture.
...
Cascade closure for posture guarantees every team has an effective posture; the platform default is the floor.
...
- [Sideband detection](/guides/sideband-detection) — tuning coherence/fault_line/fleet via posture
- [Posture cloning workflow](/guides/posture-cloning) — clone-and-customize end-to-end
