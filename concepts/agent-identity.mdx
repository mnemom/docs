...
All agent-specific API endpoints accept both `mnm-*` and `smolt-*` formats interchangeably. The unified agent card has been split into two — `alignment-card` (autonomy, integrity, conscience) and `protection-card` (front-door and back-door inspection, including the tool-result surface screened inside each request) — so use the appropriate path for what you want to read:
...
Claim is **idempotent for the same owner**: re-claiming an agent you already own returns `200` (the original `claimed_at` is preserved; pass a new `org_id` to re-home it). An agent already owned by *someone else* is **not** adoptable — that returns `409 agent_cross_tenant` (holding a valid proof is necessary but not sufficient; the agent's ownership is a resource-state conflict, not a permission failure, so it's a conflict rather than a `403`).
...
# --org takes a slug or org_id and defaults to your ACTIVE org (set via
# `mnemom org use`); with no active org set, it falls back to your personal org.
...
mnemom agent claim <mnm-id> --org acme --key "$AGENT_PROVIDER_KEY" --name my-agent
