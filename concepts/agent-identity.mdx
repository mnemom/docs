...
The two paths above diverge on *when you authenticate*. Path 2 authenticates at create time, so ownership and org placement happen immediately. Path 1 (and any agent that reached Mnemom by gateway traffic before its operator had an account) provisions the agent **unclaimed, in the Mnemom Sandbox holding org** — there is no authenticated owner yet.

`POST /v1/agents/{id}/claim` is how that agent gets an owner and a real org. It is the canonical second half of the gateway-first lifecycle:

```
gateway first-traffic  →  Mnemom Sandbox (unclaimed)  →  claim  →  your org
```

To claim, you prove possession of the agent's key (the same `hash_proof` you'd compute for Path 2) and, optionally, name the org to place it in:

- **Choose an org** — pass `org_id`. You must be a member; claiming into an org you don't belong to is rejected with `403` and the list of orgs you *can* claim into.
...
Claim is a one-time transition: an already-claimed agent returns `409`.

<Note>
Don't know your org IDs? List them with `GET /v1/orgs` (or `mnemom org list`). `GET /v1/me/context` returns your active org plus your full membership list. The same set is echoed in the `403` body when you try to claim into an org you're not a member of.
</Note>

{/* TODO(spec): insert verified curl + SDK + CLI claim examples here once gatekeeper's OpenAPI spec for the org_id contract merges and matrix posts the SDK/CLI shapes. Held until then so doc-examples spec-validation stays green. */}
