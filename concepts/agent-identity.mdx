...
- **Choose an org** — pass `org_id`. You must be a member (`owner`, `admin`, or `member`); claiming into an org you don't belong to is rejected with `403 not_a_member` and `details.claimable_orgs` — the orgs you *can* claim into, as `{ org_id, slug, name }` objects.
...
<Note>
Don't know your org IDs? List them with `GET /v1/orgs` (or `mnemom org list`). `GET /v1/me/context` returns your active org plus your full membership list. The same set is echoed in `details.claimable_orgs` when you try to claim into an org you're not a member of.
</Note>
