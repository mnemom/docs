...
- **You are the agent → self-register with [`POST /v1/agents`](/api-reference/endpoint/post-agents)** (Path 2 above). Your provider key *is* the agent's identity. You create (or idempotently re-fetch) an agent you run yourself; it's owned by you and placed in your org in one authenticated call. No holding pen, no separate claim step — you authenticated up front and the identity derives from your key.
