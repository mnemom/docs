...
interface TeamCoherenceResult {
  ...
  integrity_divergence: Array<{ agent_id; enforcement_mode: "observe" | "nudge" | "enforce" | null }>;
...
- Each pair also carries a **`confidence`** label — `"high"` when evidence comfortably clears the minimum, `"moderate"` when it just clears it, `"insufficient_evidence"` when `governance_score` is `null` — so a UI can visually distinguish a well-evidenced 0.85 from a thinly-evidenced one.
...
- **`integrity_uniform`** — `true` if all agents share the same unified-card `integrity.enforcement_mode` (`observe`, `nudge`, or `enforce`). `false` if modes differ; `null` if any card lacks an `integrity` section.
...
Fault-line classification — grouping divergences into `resolvable` / `priority_mismatch` / `incompatible` / `complementary` buckets and surfacing structural fault lines — is a **separate** layer built on top of coherence scoring. It is emitted by the `/v1/teams/fault-lines` endpoint alongside the v2 coherence vector.
