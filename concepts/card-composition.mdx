...
- An advisory written by a posture-driven detector lands in [`pending_advisories`](/concepts/integrity-checkpoints#pending-advisories) tagged with `source: sideband.{coherence,fault_line,fleet}`. The gateway picks it up on the next turn for an affected agent and lets card-driven enforcement decide the runtime treatment.
