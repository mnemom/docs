...
<Note>
**Team scope is optional.** Solo agents (in zero teams) compose under the 3-layer cascade `platform → org → agent` — the historical shape, unchanged. Teamed agents compose under `platform → org → {team₁, …, tₙ} → agent`. See [Team Scope](/concepts/team-scope) for the team layer's mechanics; the rest of this page covers the rules that apply at every layer present.
</Note>
...
| **Team** *(optional)* | Sub-org grouping for heterogeneous fleets | `teams.card_template` + `teams.protection_template` (per team) | Org owner / admin, or a delegated [Team Admin](/concepts/team-admin-role) | No — only when the agent is a team member |
| **Agent** | Per-agent specialization | `alignment_cards.card_json` (active) + `sh_configs.config_json` (protection) | Agent owner | Yes |

Scope ordering is **platform → org → (team)? → agent**. The team layer is conditional: agents in zero teams compose under the 3-layer cascade; agents in one or more teams pick up a Team layer between Org and Agent. Multi-team agents fold strictest-wins across all their teams. See [Team Scope](/concepts/team-scope) for details.
...
| `conscience.mode` | **`replace` beats `augment`** — if the org or any team sets `replace`, replace wins | Replace is the stronger commitment |
| `conscience.values` | **Mode-dependent.** `augment` (the default): union with dedup by `type:content` across platform + org + every team + agent. `replace`: union of **org + team values only** — platform's and the agent's own values are dropped. | An org/team declaring `replace` is asserting its own conscience values supersede the platform default set for this cascade, not merely adding to it — know this before setting `mode: replace` |
...
| `autonomy.bounded_actions` | **Union** across platform + org + every team + agent | Any scope can grant a bounded action; the agent's own declaration adds to, not replaces, what platform/org/team already granted |
...
| `capabilities.<name>.tools[]` | **Union** across org + every team + agent, per capability name | A capability's grant only ever grows down the cascade |
| `capabilities.<name>.severity_on_unmapped` | **Strictest wins** (`critical` > `high` > `medium` > `low`) | Any scope can raise the bar for an unmapped tool under this capability |
| `capabilities.<name>.description` | **Agent wins**, falling back to the last team that defined it, then org | Descriptions are local, editorial context |
| `enforcement.allow_unmapped_tools` | **Strictest wins (`false` over `true`)** | Any scope can require mapped tools |
| `audit.retention_days` | **Max across scopes** (longest retention wins) — the one exception to the platform-scoped rule below | A team/org/agent can promise *longer* retention than the platform floor |
| `audit.trace_format`, `audit.queryable`, `audit.tamper_evidence`, `audit.query_endpoint` | **Platform-scoped** — org/team/agent values are accepted but ignored | Audit is the substrate every governance promise rests on; letting downstream scopes strengthen it on paper only would produce hollow promises |
| `audit.storage.*` | **Dropped from the composed output entirely** — no consumer reads it | Authored `storage` config isn't preserved past composition today |
...
Between an org-template change and the recompose job finishing, the canonical card is stale. The gateway handles this with a `needs_recompose` cache bypass: when the canonical row's flag is true, the gateway serves the canonical without populating its short-lived read cache, so the next change after recompose is seen immediately.
...
Postures and cards are parallel artifact types: postures are team-scoped policy input (drives the observer's sideband sweep), cards are agent-scoped runtime treatment (read by the gateway at request time). They share infrastructure (this composer idiom, read-through caching, audit log) but their data planes are independent.

Practically:

- An advisory written by a posture-driven detector lands in [`pending_advisories`](/specifications/pending-advisories-schema) tagged with `source: sideband.{coherence,fault_line,fleet}`. The gateway picks it up on the next turn for an affected agent and lets card-driven enforcement decide the runtime treatment. This next-turn pickup is specific to *advisories*; screening is not deferred that way. The protection card's `screen_surfaces` are evaluated inside the request that carries them — including tool results, which are screened at the front door before the same request is forwarded. See [When the front door runs](/concepts/safe-house#when-the-front-door-runs).
