...
They are deliberately distinct from the agent-facing advisories the gateway carries forward into an agent's next turn (the `pending_advisories` surface — see the [wire format](/specifications/pending-advisories-schema)). The architectural commitment is:
...
2. **Notification destinations**: `governance.signal.fired`, `governance.signal.acknowledged`, `governance.signal.resolved`, `governance.signal.dismissed`, `governance.escalation.triggered`. A generic `webhook` destination delivers an HMAC-SHA256 signed POST (`X-Mnemom-Signature: sha256=<hex>` over the raw JSON body, plus `X-Mnemom-Event` and `X-Mnemom-Delivery-Id` headers). This is a purpose-built delivery path for governance destinations, configured per-org and separate from the general-purpose org webhook subscriptions described in the [webhook contract](/concepts/webhook-contract) — do not assume the same signature scheme or retry schedule applies.
...
- **`webhook`** — generic HMAC-signed POST to a URL of your choice (see the signature format above).
