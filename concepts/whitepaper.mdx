...
Not all upstreams carry the same risk. Per ADR-037, `trusted_sources` on the protection card is a typed allowlist with three buckets — `domains` (DNS names), `agent_ids` (Mnemom agent identifiers for agent-to-agent traffic), and `ip_ranges` (IPv4/IPv6 CIDR). A match on any bucket short-circuits detection for that message; each skip emits a low-priority `sh_trusted_source_skip` audit trace so reviewers can see what was waved through. Trust is binary by design — graduated per-source multipliers were considered during the schema unification and rejected as an attack surface (operator misconfiguration risk dominates the precision win). Sensitivity tuning lives in `thresholds.{warn, quarantine, block}` instead.

#### 5.3.7 Modes

Safe House modes follow the canonical four-value `ProtectionMode` enum, mirroring the alignment card's `integrity_mode`:

- **`off`.** Detection skipped entirely. No telemetry. Used for cost / latency / non-applicability cases.
- **`observe`.** Detectors run, signals logged asynchronously, no request-path action. Verdicts and session risk scores appear in traces.
- **`nudge`.** Detectors run synchronously; matches attach an advisory annotation to the agent's prompt context (and an `X-Safe-House-Advisory` response header) but the request proceeds. The agent sees that something was flagged and can decide whether to comply.
- **`enforce`.** Detectors run synchronously; matches block the request (`quarantine ≥ quarantine threshold`, hard block at `≥ block threshold`).

Customers typically deploy through this sequence: `observe` first to map the threat landscape with no customer impact, then `nudge` to add a model-in-the-loop signal, then `enforce` to make it a control. Composition across scopes follows strictest-wins: `enforce > nudge > observe > off`. An agent cannot drop below the platform/org floor.
...
4. EU AI Act. Regulation (EU) 2024/1689. European Parliament and Council, 2024.
5. NIST NCCoE Concept Paper: "Accelerating the Adoption of Software and AI Agent Identity and Authorization." February 2026.
6. ISO/IEC 42001:2023. Artificial Intelligence — Management System.
7. ISO/IEC 42005:2025. Artificial Intelligence — AI Impact Assessment.
8. IEEE 7001-2021. Transparency of Autonomous Systems.
9. IEEE 3152-2024. Transparent Human and Machine Agency Identification.
10. Singapore IMDA Model AI Governance Framework for Agentic AI. January 2026.
11. OpenTelemetry GenAI Semantic Conventions. CNCF, 2025.
12. NIST SP 800-207. Zero Trust Architecture. August 2020.
13. NIST SP 800-63-4. Digital Identity Guidelines. 2024.
14. Succinct Labs. "SP1 zkVM Documentation." 2025.
15. C2PA (Coalition for Content Provenance and Authenticity). "C2PA Technical Specification." 2025.
16. W3C. "Verifiable Credentials Data Model v2.0." W3C Recommendation, 2024.
17. NIST. "Artificial Intelligence Risk Management Framework (AI RMF 1.0)." NIST AI 100-1. January 2023.
18. World Economic Forum. "Presidio AI Framework: Towards Safe Generative AI Models." 2024.
19. World Economic Forum. "Navigating the AI Frontier: Agent Governance." AI Governance Alliance. January 2026.
20. ERC-8004. On-Chain Reputation Registry Standard. Ethereum Improvement Proposals.
21. Merkle, R. C. "A Certified Digital Signature." *Advances in Cryptology — CRYPTO '89*. Springer, 1989.
22. Bernstein, D. J., Duif, N., Lange, T., Schwabe, P., and Yang, B.-Y. "High-speed high-security signatures." *Journal of Cryptographic Engineering*, 2(2):77–89, 2012.
23. FIPS 180-4. "Secure Hash Standard (SHS)." NIST, August 2015.
24. Ben-Sasson, E., Bentov, I., Horesh, Y., and Riabzev, M. "Scalable, transparent, and post-quantum secure computational integrity." IACR Cryptology ePrint Archive, 2018.
25. Goldwasser, S., Micali, S., and Rackoff, C. "The Knowledge Complexity of Interactive Proof Systems." *SIAM Journal on Computing*, 18(1):186–208, 1989.
