...
When AI agents act on behalf of humans, they bring equal reasoning power, equal tool access, and equal information retrieval capabilities. An AI agent shopping for a consumer has the same access to product databases, reviews, and pricing data as the merchant's AI. The asymmetry that powered trillion-dollar advertising businesses evaporates when agents negotiate with agents.
...
It is tempting to ship three independent products. Many incumbents do — a logging tool, a guardrail tool, and a WAF. The tempting story is false for three reasons.
...
Cryptographic attestation closes this gap. It transforms oversight verdicts from claims into evidence — evidence that can be verified by any party, without trusting the Mnemom API, without trusting the analysis service, without trusting anything except mathematics.
...
Stage 1 is prohibitively expensive to prove cryptographically with current zero-knowledge technology. Stage 2 is approximately 10,000 RISC-V cycles — trivial for a zkVM.
...
The guarantee is precise: *"Given these concerns, the verdict derivation was honest."* The non-guarantee is equally precise: *"The concerns themselves were correctly identified."* Both are communicated to verifiers. This honesty is what makes the layer credible.
...
The natural first instinct is to compute a single "fleet coherence score" from pairwise comparisons. The platform ran this at first. In production, on real fleets, single-number fleet scores were **actively dishonest**:

1. **Silence counted as disagreement.** A value declared by one agent that another agent does not mention deflates a Jaccard-flavored score — even though absence from a role-specialist card is specialization, not disagreement.
2. **Role specialization was punished.** A monitoring agent and a remediation agent sharing all seven governance commitments but differing on five role-specific values landed at ≈58% coherence — not because they conflict, but because the denominator counted every unique value as potential disagreement.
...
Absence of a single fleet score is not a gap; it is the contribution. Consumers that require a scalar must derive one from the vector themselves and take responsibility for that compression. See [Fleet Coherence](/concepts/fleet-coherence).
...
No single layer provides complete protection. The layered design is the point. Ordered from fastest to slowest:
...
The agentic internet is arriving with or without a trust plane. The question is not whether agents will coordinate — they already are — but whether that coordination will be observable, auditable, contextually defended, and *provably* honest.

This paper frames Mnemom as a single governance stance, composed of three oversight layers and a cryptographic attestation system, unified behind a managed gateway and one declared contract.

**Post-hoc behavioral oversight (AAP).** Alignment cards declare what the agent claims. AP-Traces record what the agent did. Drift detection tracks divergence across sessions. This layer makes behavior *observable* — but the agent controls what it logs.

**Real-time cognitive oversight (AIP).** Integrity checkpoints reveal what the agent was thinking. Conscience prompts evaluate reasoning against declared values. Side-channel signals enable intervention before harm occurs. This layer makes intent *analyzable* — but the analysis engine is a new trust dependency.

**Contextual oversight (Safe House).** Inbound screening catches what reaches the agent; outbound screening catches what leaves it. Canaries convert silent compromise into loud evidence. This layer extends oversight across the agent's context boundary — but depends on pattern coverage and adversary sophistication.

**Cryptographic attestation.** Ed25519 signatures prevent verdict fabrication. Hash chains prevent retroactive modification. Merkle trees prevent selective deletion. SP1 STARK proofs prove the deterministic judgment was applied honestly. Base L2 anchors make the attestation chain independently verifiable. This layer makes oversight *verifiable* — removing the need to trust the oversight service itself.

The three oversight layers address the three failure modes a trust plane owes the agentic internet. The attestation chain removes the recursive trust problem that each oversight layer introduces. The unified alignment contract prevents the modes from fragmenting into three disjoint products. The managed gateway makes all of it reachable with a single configuration change.

The critical conceptual move of the attestation layer is the auditor-model distinction. By proving the deterministic judgment rules rather than the LLM inference, we produce a system that is practical at per-checkpoint cost, fast enough for near-synchronous delivery, and meaningful (the verdict derivation was honest) — without waiting for the still-distant day when full inference proofs become viable for frontier models.

The infrastructure is open. Protocol specifications, schemas, SDKs, the dimensional team-coherence scorer, and the on-chain registry ABIs are publicly available. Certificates are publicly verifiable. Limitations are stated.
