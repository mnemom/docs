...
Previously, Safe House read a JSON `sh_configs` row keyed by agent_id. The automatic migration composed every existing `sh_configs` row into a protection-card YAML and wrote the canonical version.

- Legacy endpoint `GET/PUT /v1/agents/:id/cfd/config` is **removed**. Use `GET/PUT /v1/agents/:id/protection-card`.
- The internal `SafeHouseConfig` TypeScript type that Safe House detectors consume is stable; the consolidation swapped the fetch source, not the type. If you're embedding Safe House detectors, your integration did not need to change.
