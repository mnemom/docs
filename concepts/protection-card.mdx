...
[AEGIS Managed Rules](/concepts/managed-rules) tune protection-card behavior across the network. When a new Managed Rule promotes platform-scope (Ed25519-signed; tier-3 auto-promoted with 24h observe soak; tier-1/-2 dual-control), the gateway loads it via a tiered, multi-layer read substrate and adds the rule's detection thresholds to the screening pipeline your protection card configured. Your card's `mode` (`off / observe / nudge / enforce`) determines what action the gateway takes when a Managed Rule matches.
