---
title: "Mnemom Trust Score"
description: "The Mnemom Trust Score™ — a credit score for AI agents. A composite trust metric built from integrity checkpoints, drift stability, trace completeness, and fleet coherence."
sidebarTitle: "Mnemom Trust Score"
icon: "ranking-star"
---

## Overview

The Mnemom Trust Score™ is a **composite trust metric for AI agents** — the equivalent of a credit score, but for autonomous software. It answers a question no other system answers: *Based on independently verified behavioral evidence, how trustworthy is this agent?*

Unlike self-reported trust claims or capability benchmarks, the Mnemom Trust Score is:
...
Mnemom Trust Scores power trust decisions across the AI agent ecosystem: pre-interaction trust checks, fleet governance policies, compliance reporting, marketplace listings, and inter-agent delegation via [A2A](/protocols/aap/a2a-integration).

<Note>
The Mnemom Trust Score requires a minimum of **50 analyzed integrity checkpoints** before a public score is published. This minimum prevents gaming through selective checkpoint submission and ensures statistical significance.
</Note>
...
```
S = w_1 * integrity_ratio + w_2 * violation_recency + w_3 * drift_stability
...
### Violation Recency (20%)

Recent violations penalize more heavily than old ones. Uses exponential decay with a **1-week half-life** -- a violation from two weeks ago has one-quarter the impact of a violation today.

```
score = 1000 * (1 - max_recent_impact)
```

This component incentivizes continuous improvement: even agents with a poor history can recover as violations age out.

- **Data source:** AIP checkpoints with `boundary_violation` verdict
...
if (!reputation || reputation.grade === 'CCC' || reputation.grade === 'NR') {
  return escalateToPrincipal({
    reason: `Agent ${theirAgentId} has insufficient trust score (${reputation?.grade ?? 'NR'})`,
  });
}
...
Every Mnemom Trust Score is backed by a cryptographic proof chain that enables independent verification without trusting Mnemom's infrastructure. The verification endpoint at `GET /v1/reputation/{agent_id}/verify` returns the full proof chain.
