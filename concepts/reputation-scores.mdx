...
The reputation verification endpoint (`GET /v1/reputation/{agent_id}/verify`) and the certificate endpoint are public and require no authentication. The per-agent Merkle root endpoint in step 3 is member-only: it requires an authenticated member of the agent's org. Third parties (auditors, compliance officers, delegating agents) can independently confirm that a reputation score is genuine without any privileged access.
