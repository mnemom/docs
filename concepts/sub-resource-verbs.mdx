...
Writes don't blindly accept your patch. The composer reads the current full spec, splices your primitive in, then validates the merged result against the [unified card schema](/concepts/unified-cards-overview).
...
- **CI Action edits a single primitive** — the upcoming [`mnemom/cards-action`](/guides/manifest-authoring-with-scaffold) drives sub-resource verbs from per-file YAML diffs.
...
- [Unified Cards](/api-reference/unified-cards-overview) — the alignment + protection spec.
