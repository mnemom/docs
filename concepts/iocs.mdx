...
description: "The STIX 2.1 Indicator-of-Compromise feed AEGIS publishes at /v1/trust/iocs. Empty when the network is calm; populated as AEGIS detects."
sidebarTitle: "IoCs"
icon: "list-radio"
---
...
The feed is the [L5 layer](/concepts/protection-network#l5-ioc-feed-and-advisories) of the Protection Network — the public transparency surface that lets external SIEMs, threat-intel platforms, and partner networks consume the same indicators AEGIS pushes to every gateway internally.
...
## Empty by design when the network is calm
...
When nothing has been published, the bundle is `{ "type": "bundle", "id": "bundle--<uuid>", "objects": [] }`. Customers polling the feed see an empty `objects` array; the field is reliable — it means no indicators, not a broken endpoint.

When AEGIS publishes a real IoC, it carries `mnemom_synthetic: false` in the Mnemom extension. The synthetic flag is the field customers can rely on to distinguish a real published indicator from any illustrative or test-seed entry. See [AEGIS — calm-at-GA contract](/concepts/aegis#the-calm-at-ga-contract).
...
- Refresh cadence (recommended 5-15 minutes via cron; rate limit 1 req/sec/IP)
...
New IoC insertions are not delivered as a customer webhook today; poll the IoC feed described in [IoC feed consumption](/guides/ioc-feed-consumption). The customer-subscribable events are listed on the [Webhook events](/api-reference/webhook-events) page.
