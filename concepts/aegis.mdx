...
**Mnemom AEGIS** — Adaptive Enforcement, Governance & Intelligence Substrate — is the protection layer of [Safe House](/concepts/safe-house). It screens every agent transaction at four checkpoints (front door, back door, inside.autonomy, inside.integrity), each independently set to one of four enforcement modes (`off`, `observe`, `nudge`, `enforce`). A checkpoint is a position, not a single evaluation — the front door alone screens the inbound message *and* every tool result the request carries, inside the same turn. AEGIS is the cross-tenant defensive network above the per-customer perimeter: per-customer Safe Houses detect on what they can see; AEGIS sees across all of them and feeds signed Managed Rules back to every gateway in the network.
...
A checkpoint is a *position*, not a single evaluation. The front door in particular fires more than once per turn: it screens the inbound message when the request arrives, and then screens each tool result the request carries turn-internally — inline, one result at a time, before the request is forwarded. See [Turn-internal tool-result screening](/concepts/safe-house#turn-internal-tool-result-screening).

### Four checkpoints

| Checkpoint | Position | What it screens |
|---|---|---|
| **Front door** | Inbound — the message at request arrival, plus each tool result turn-internally, mid-request (enforce: before the content reaches the agent; observe/nudge: post-hoc) | Prompt injection, indirect injection (per tool result, with an up-only session-posture ratchet across the results in a turn), social engineering, BEC fraud, agent spoofing |
...
| **`enforce`** | Synchronous action at quarantine / block thresholds — the request is blocked, or, on the tool-result surface, the offending result is withheld and the turn continues. |
