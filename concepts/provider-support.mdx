...
This is the load-bearing honesty disclosure. Mnemom does not claim uniform AIP coverage across providers. See [SLI-P1 in the Safe House SLOs](https://github.com/mnemom/safe-house-hardening/blob/main/slos.md#sli-p1-aip-coverage-by-provider) for the public commitment.
...
<sup>[*]</sup> "AIP coverage" is defined as the percentage of customer-facing AIP analysis cycles where the verifier had full thinking-trace inspection. Anthropic and Gemini expose full traces; OpenAI o-series exposes reasoning summaries only; non-reasoning OpenAI models expose no internal reasoning. See [SLI-P1](https://github.com/mnemom/safe-house-hardening/blob/main/slos.md#sli-p1-aip-coverage-by-provider) for the rolling 30-day measurement methodology.
...
Per-provider P50/P95 latency for the Safe House dispatch path and AIP analysis is published on the [status page](https://status.mnemom.ai) and committed via [SLI-P2](https://github.com/mnemom/safe-house-hardening/blob/main/slos.md#sli-p2-per-provider-dispatch-overhead) and [SLI-P3](https://github.com/mnemom/safe-house-hardening/blob/main/slos.md#sli-p3-per-provider-aip-analysis-latency).
...
Mid-stream interruption for integrity violations is not supported on any provider (per [ADR-040](https://github.com/mnemom/scale/blob/main/decisions/ADR-040-same-turn-enforcement.md)). AIP runs after the upstream response completes, but before the gateway delivers it to the customer.
...
For Anthropic and Gemini, **both layers run**. For OpenAI, only the static layer runs today — see the disclosure below.

<Warning>
**OpenAI live tests are currently paused.** Mnemom's OpenAI safety-research test account was deactivated by OpenAI's automated abuse-detection systems (the safety-evaluation traffic patterns Mnemom legitimately uses to test guardrails look identical to attacker traffic from the outside). We are working with OpenAI to restore the account. While the account is suspended:

- **OpenAI adapter correctness is still verified** on every PR via captured response fixtures.
- **Live verification of OpenAI changes** does not run nightly. If OpenAI ships a breaking change to their streaming format, Mnemom would detect it on the next captured-fixture refresh (quarterly cadence by default; can be triggered manually within 24 hours).
- **OpenAI inference itself is unaffected.** Customer traffic to OpenAI through the gateway works as documented.

This is disclosed publicly because Mnemom's commitment is honest per-provider differentiation. Anthropic and Gemini get nightly upstream-breakage detection; OpenAI does not until the account is restored.
</Warning>
...
Provider expansion beyond Anthropic + OpenAI + Gemini is **not in scope for v1**. Cohere, Mistral, Together, Groq, and other providers are not supported. Adding a provider is a multi-quarter effort (Safe House dispatch, AIP adapter, CLPI policy schema mapping, harness coverage, docs) — tracked separately.

[BYOK (bring-your-own-key)](https://github.com/mnemom/scale/blob/main/decisions/ADR-042-headers-rationalization.md) for upstream providers is also out of v1. v1 ships with Mnemom-only key custody.
