...
- `field_provenance` (the [caller-aware redaction](/concepts/agent-state#caller-aware-redaction) is server-side; A2A consumers don't get a redacted view, they get no view)
- Internal connector grants beyond the bare tool name surface

If you'd benefit from the full composed view including provenance, use [`/v1/agents/{id}/state`](/concepts/agent-state) under an authenticated principal.
...
```bash
curl -X PATCH https://api.mnemom.ai/v1/agents/<agent_id>/settings \
