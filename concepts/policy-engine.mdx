...
Policies are defined in a YAML-based DSL (domain-specific language) with five top-level sections: `capability_mappings`, `forbidden`, `defaults`, and optionally `escalation_triggers` and `meta`. They are evaluated in three contexts -- CI/CD pipelines, live gateway requests, and post-action observation -- giving operators governance coverage across the full agent lifecycle.

<Note>
  The policy engine is a parallel enforcement layer to [alignment enforcement](/gateway/enforcement) (observe/nudge/enforce). Alignment enforcement checks agent behavior against card values. Policy enforcement checks tool usage against policy rules. Both run independently and produce separate verdicts.
</Note>
...
mnemom policy validate --file policy.yaml

# Evaluate policy against historical traces
mnemom policy evaluate --file policy.yaml --agent-id agent_abc123 --days 30
```
...
**Use case:** Pre-deploy gates. A CI job runs `mnemom policy validate` and `mnemom policy evaluate` before merging policy changes. If evaluation shows violations in recent traces, the merge is blocked.
...
Live evaluation runs in real-time as requests pass through the Mnemom gateway. The policy engine extracts tool names from the request body, checks each tool against the policy, and returns a verdict.
...
- [Policy CLI Reference](/gateway/policy-cli) -- CLI commands for validating, evaluating, and managing policies
...
- [Enforcement Modes](/gateway/enforcement) -- Alignment enforcement modes (observe/nudge/enforce)
