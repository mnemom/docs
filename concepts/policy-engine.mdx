...
**Policy lookup:** The gateway resolves the agent's policy by looking up the agent ID from the request metadata. If the primary ID lookup (e.g., a `smolt-*` session ID) returns no policy, the gateway falls back to the agent name from the `x-smoltbot-agent` header. This ensures policy enforcement works for agents regardless of which ID format the gateway assigns.
