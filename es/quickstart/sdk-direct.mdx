...
<CodeGroup>
```bash Python
pip install agent-alignment-protocol agent-integrity-protocol
```
...
for (const alert of alerts) {
  ...
  console.log(`  Direction: ${alert.drift_direction}`);
  console.log(`  Similarity: ${alert.similarity_score}`);
  console.log(`  Sustained for ${alert.sustained_traces} traces`);
}
```
</CodeGroup>
