...
- Comprobaciones de integridad AIP por turno
...
<Steps>
  ...
  <Step title="Lo que querrás leer de vuelta">
    ...
    | `X-Mnemom-Verdict` | Siempre (gateway) | Estructurado `front=…; autonomy=…; integrity=…; back=…` con cada valor en `{pass \| observed \| nudged \| enforced \| unverified}`. Analízalo; el estado de cuatro puntos de control te indica qué observó Safe House (front+back), qué hizo CLPI en las llamadas a herramientas (autonomy) y qué hizo AIP en el razonamiento (integrity). `unverified` es exclusivo de integrity: el analizador falló, así que no existe un veredicto confiable — `enforce` retiene la respuesta (fail-closed, sigue siendo 2xx), `observe`/`nudge` la reenvían y registran el estado unverified. Nunca se reporta como `pass`. |
    ...
    if (checkpoints.integrity === 'unverified') {
      // El analizador falló — no hay veredicto confiable. En enforce esta
      // respuesta ya fue retenida/reemplazada; en observe/nudge fue reenviada.
    }
    ```
...
- **Integridad** -- Análisis AIP por turno de los bloques de pensamiento, con veredicto (`clear` / `review_needed` / `boundary_violation`)
...
| `enforce` | Aplica el veredicto en el mismo turno tanto para solicitudes con streaming como sin streaming (HTTP 403 para las solicitudes sin streaming); en streaming esto añade latencia porque la respuesta se evalúa antes de entregarse. |
