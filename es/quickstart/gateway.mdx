...
- Comprobaciones de [integridad AIP](/concepts/integrity-checkpoints) en tiempo real
...
- Verificación frente a la [tarjeta de alineamiento](/concepts/agent-cards) del agente
...
<Steps>
  ...
  <Step title="Lo que querrás leer de vuelta">
    ...
    Consulta la [referencia de encabezados](/api-reference/headers) para el conjunto canónico completo + parsers por lenguaje, y la [referencia de errores](/api-reference/errors) para el mapeo veredicto-a-estado (cuando `enforced` se convierte en cuarentena 422 o bloqueo 403).
  </Step>
  ...
  <Step title="Reclamar tu agente">
    ...
    <Note>
      Una respuesta `503` significa que tu organización personal aún está siendo aprovisionada. Espera unos segundos y vuelve a intentarlo. Para errores `403` de inquilino cruzado o no-miembro, consulta la [guía del flujo de reclamación de agentes](/guides/agent-claim-flow).
    </Note>
  </Step>
...
Si ejecutas múltiples agentes detrás de la misma clave API, usa el encabezado `x-mnemom-agent` para dar a cada uno una identidad distinta. La ruta del proveedor permanece sin cambios — la pasarela deriva un ID de agente único mediante `SHA256(apiKey + '|' + agentName)`. Consulta [Identidad del agente](/concepts/agent-identity) para la derivación completa del ID, las rutas de creación automática vs registro programático y cómo la rotación de claves interactúa con la identidad del agente.
...
<Tip>
También puedes crear agentes de forma programática mediante la [API CRUD de agentes](/api-reference/endpoint/post-agents) si quieres pre-crearlos con metadatos antes de su primera solicitud.
</Tip>
...
- [Explorar conceptos](/concepts/alignment-cards) para entender las tarjetas de alineamiento, las trazas y la integridad
...
- [Leer sobre la aplicación](/gateway/enforcement) para documentación detallada de los modos de aplicación
- [Auto-alojar](/quickstart/self-hosted) si necesitas control completo de residencia de datos
