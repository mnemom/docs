...
<Steps>
  ...
  <Step title="Iniciar la stack">
    ...
    Esto inicia cinco servicios en orden:
    1. **PostgreSQL** — base de datos con comprobación de salud
    2. **Redis** — capa de caché con persistencia
    3. **Migrate** — aplica el esquema de la base de datos (se ejecuta una vez, luego termina)
    4. **Gateway** — proxy HTTP en el puerto 8787
    5. **Observer** — planificador en segundo plano para el procesamiento de trazas
  </Step>
...
|---|---|
| `SUPABASE_URL` | URL del proyecto Supabase o endpoint REST de PostgreSQL |
| `SUPABASE_KEY` | Clave de rol de servicio de Supabase |
...
Las migraciones se ejecutan automáticamente mediante el servicio `migrate`.
