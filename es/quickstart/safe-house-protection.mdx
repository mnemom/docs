...
Las superficies son unidades de filtrado, no un presupuesto por turno: la puerta de entrada se ejecuta una vez por cada superficie habilitada que transporte la solicitud. La tarjeta de abajo empieza solo con `incoming`, así que verás una evaluación de puerta de entrada por solicitud. Activar `tool_responses` ([Paso 7](#paso-7--filtrar-los-resultados-de-herramientas)) añade otra evaluación por cada resultado de herramienta que la solicitud devuelve al modelo — dentro de esa misma solicitud, antes de que el modelo lo lea.
...
## Paso 7 — Filtrar los resultados de herramientas

Los pasos 1–6 solo filtraron `incoming` — el mensaje del usuario. Si tu agente usa herramientas, la vía de inyección más habitual es el *resultado de herramienta*: un resultado de búsqueda, el cuerpo de un correo, una respuesta de API con instrucciones ocultas dentro. Activa la superficie `tool_responses`:
...
screen_surfaces:
  ...
  outgoing: false
  tool_calls: false
  tool_responses: true      # filtrar los valores que devuelven las herramientas
```

A partir de aquí, una solicitud que devuelve resultados de herramientas al modelo se filtra **de nuevo dentro de esa misma solicitud** — una vez por cada resultado de herramienta, en su propia superficie, antes de que el cuerpo se reenvíe al proveedor. Es una comprobación de puerta de entrada; nada espera al siguiente turno del agente. Sin embargo, la cobertura no es incondicional: una solicitud que se ramifica hacia muchas llamadas a herramientas en un solo turno no tiene garantizada la cobertura completa.

La forma de aplicación difiere de la del Paso 5. Un *mensaje* marcado hace fallar la solicitud con un 422 o un 403. Un *resultado de herramienta* marcado no hace fallar la solicitud — se retira del cuerpo:

| Veredicto por herramienta | Lo que ves |
|---|---|
| `pass` | El resultado de herramienta se reenvía sin cambios. |
| `warn` | El resultado de herramienta se entrega, anotado como no confiable. |
| `quarantine` / `block` | El contenido del resultado de herramienta se sustituye por un aviso de cuarentena; el modelo nunca ve la carga útil. La solicitud sigue devolviendo **200**, con `front=enforced` en `X-Mnemom-Verdict`. |
...
<Tip>
No interpretes un 200 como «la puerta de entrada no hizo nada en este turno». Analiza `X-Mnemom-Verdict` — `front=enforced` en un 200 significa que un resultado de herramienta fue retenido o anotado dentro de la solicitud.
</Tip>
...
- [Cuándo se ejecuta la puerta de entrada](/concepts/safe-house#when-the-front-door-runs) — Por qué la puerta de entrada se dispara una vez por superficie de entrada, no una vez por turno
