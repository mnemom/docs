...
Named agents use the `x-smoltbot-agent` header to specify an agent name (e.g., `x-smoltbot-agent: my-coder`). The provider path stays unchanged (`/anthropic/v1/messages`). The gateway hashes `SHA256(apiKey + '|' + agentName)` to derive a unique identity for each named agent, so multiple agents can share the same API key while maintaining distinct trace histories.
...
- **Multi-agent support** — Multiple named agents can share a single API key with distinct identities using the `x-smoltbot-agent` header. See the [Multi-Agent Setup](/guides/multi-agent-setup) guide and the [Agent CRUD API](/api-reference/endpoint/post-agents) for programmatic registration.
