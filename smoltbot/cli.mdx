...
```bash
smoltbot agents rekey            # rekey the default agent
smoltbot agents rekey my-coder   # rekey a named agent
```
...
**CI/CD**: Set `SMOLTBOT_NEW_KEY` to skip the prompt:

```bash
SMOLTBOT_NEW_KEY=sk-ant-... smoltbot agents rekey
```

See [Rotating Your Agent's API Key](/guides/agent-key-rotation) for the full guide.

### `smoltbot agents check-binding [agent-name]`

Verify that a specific API key is currently bound to an agent. The key is hashed locally — never transmitted.

```bash
smoltbot agents check-binding            # check default agent
smoltbot agents check-binding my-coder   # check a named agent
```
...
**CI/CD**: Set `SMOLTBOT_CHECK_KEY` to skip the interactive prompt.
