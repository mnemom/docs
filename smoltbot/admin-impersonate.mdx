...
<Steps>
  <Step title="Start a Session">
    Click **View as Customer** from the customer detail panel, or call `POST /v1/admin/users/:userId/impersonate`. The backend creates a session record and returns a short-lived opaque token.
  </Step>
  <Step title="View as Customer">
    The frontend stores the token and includes it as an `X-Impersonate-Token` header on all API calls. The backend resolves the target user's data instead of yours — agents, billing, org context, everything.
  </Step>
  ...
  <Step title="Exit">
    Click **Exit** in the amber banner, or wait for the session to expire after 1 hour. The token is cleared and you return to the admin panel.
  </Step>
</Steps>
...
Only admins with `app_metadata.is_admin === true` can start impersonation sessions. The admin guard is the same one used for all admin endpoints.
