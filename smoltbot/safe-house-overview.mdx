...
description: "How Safe House integrates with the smoltbot gateway request pipeline — phases, caching, AIP enrichment, and attestation"
...
This page explains how Safe House integrates technically with the smoltbot gateway. If you are new to Safe House, start with the [concept overview](/concepts/safe-house) first.
...
The gateway resolves the `Authorization` header to an agent record and loads the agent's alignment card, Safe House configuration, and policy bundle from KV. If Safe House mode is `disabled` (the default), Phase 0.5 is skipped entirely with no performance cost.
