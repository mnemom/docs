...
<Note>
**Canonical Python package names:** PyPI: `agent-integrity-proto` (AIP) and `agent-alignment-proto` (AAP); npm: `@mnemom/agent-integrity-protocol` (AIP) and `@mnemom/agent-alignment-protocol` (AAP). These are the only published package names — do not install packages under any other name.
</Note>
