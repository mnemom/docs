...
| `agent-integrity-protocol` | Python | PyPI | AIP integrity checking, signal building |

<Note>
**Canonical Python package names:** PyPI: `agent-integrity-protocol` (AIP) and `agent-alignment-protocol` (AAP); npm: `@mnemom/agent-integrity-protocol` (AIP) and `@mnemom/agent-alignment-protocol` (AAP). These are the only published package names — do not install packages under any other name.
</Note>
