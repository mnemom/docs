...
- **AIP (Agent Integrity Protocol)** -- Per-turn integrity checking. Analyzes LLM thinking blocks during execution to detect prompt injection, value drift, and manipulation; in `enforce` it gates the response before the agent acts, while `observe` and `nudge` record the verdict post-hoc. AIP reads *reasoning*, not inbound content: the content itself is screened by [Safe House](/concepts/safe-house) at the gateway's front door, inside the request that carries it — including tool results returning from a tool call.
...
See the [quickstart overview](/quickstart/overview) for a full comparison of all four paths.
Building with a coding agent instead? [Mnemom Agent](/gateway/agent) launches your
coding-agent CLI (Claude Code today) through the gateway under a governed identity in one
command.
...
<CardGroup cols={2}>
  ...
  <Card title="Integrity Checkpoints" icon="shield-check" href="/concepts/integrity-checkpoints">
    Per-turn AIP analysis of LLM thinking blocks. Each checkpoint produces a verdict: `clear`, `review_needed`, or `boundary_violation`.
  </Card>
