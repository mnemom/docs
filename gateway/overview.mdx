...
<Note>
The hosted gateway at `gateway.mnemom.ai` is what you integrate against regardless of which backend serves a given request — that endpoint, its request/response contract, and the headers documented in [Enforcement Modes](/gateway/enforcement) are the stable surface. The **self-hosted** stack ships on Cloudflare Workers + Supabase — see the [self-hosted quickstart](/quickstart/self-hosted) if you're running your own instance.
</Note>
