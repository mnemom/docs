...
<Note>
The hosted gateway at `gateway.mnemom.ai` is what you integrate against regardless of which backend serves a given request — that endpoint, its request/response contract, and the headers documented in [Enforcement Modes](/gateway/enforcement) are the stable surface. If you'd rather run your own instance, see the [self-hosted quickstart](/quickstart/self-hosted) for that deployment's infrastructure requirements -- they describe the self-hosted stack only, not how Mnemom operates the managed service.
</Note>
...
├── gateway/      # API proxy + tracing
├── observer/     # Trace builder + AAP verification
├── api/          # REST API
└── database/     # Postgres schema (self-hosted: Supabase)
```
...
Your App → mnemom   ├─── /openai/*    ──→ OpenAI (GPT-5)
   ...
   (immediate)   (async)
              ...
              AIP analysis (Haiku, ~3s)
                    |
           checkpoint + attestation stored
                    |
           Policy evaluation (autonomy_mode-gated)
                    |
           Observer (cron) → AP-Trace stored
                    |
           Dashboard (mnemom.ai)
```
...
list and per-provider feature coverage.
...
- **Protection** — Safe House front-door and back-door verdicts. The front door records one evaluation per inbound surface, so a turn that carries tool results produces several — see [When the front door runs](/concepts/safe-house#when-the-front-door-runs)
...
Coverage varies by provider — see [Provider Support](/concepts/provider-support#aip-coverage-by-provider--the-headline-commitment)
for the full matrix and honest disclosure. In short: Anthropic and Gemini get
full thinking-trace analysis; OpenAI gets none through the gateway today (its
Chat Completions door never returns reasoning content, on any model).
...
- **AIP requires exposed reasoning** — AIP integrity checking needs a provider that exposes thinking content. Anthropic and Gemini do; OpenAI's Chat Completions door does not expose any reasoning content today, on any model, so OpenAI gets synthetic `clear` verdicts rather than analysis — see [Provider Support](/concepts/provider-support).
- **Hosted gateway** — The default gateway runs on Mnemom infrastructure. Self-hosting is possible but requires manual setup.

## Dependencies

- [Agent Alignment Protocol (AAP)](/protocols/aap/specification) — [`@mnemom/agent-alignment-protocol`](https://www.npmjs.com/package/@mnemom/agent-alignment-protocol) on npm
- [Agent Integrity Protocol (AIP)](/protocols/aip/specification) — [`@mnemom/agent-integrity-protocol`](https://www.npmjs.com/package/@mnemom/agent-integrity-protocol) on npm
- API keys: Anthropic (required for AIP analysis), OpenAI and Gemini (optional, for multi-provider tracing)

If you're running the [self-hosted](/quickstart/self-hosted) stack instead of the hosted gateway, it deploys on Cloudflare Workers with a Supabase Postgres database.
