...
gateway with an agent set up for you.
</Note>
...
launcher, and the fleet verbs `list`, `claim`, and `move`.

### `mnemom agent`

Launch a governed coding-agent session (Claude Code today) through the gateway.
A bare `mnemom agent` asks for a session name and an optional goal, then
launches. The launcher, its sub-verbs (`setup`, `doctor`, `config`, `invite`),
saved settings, contracts, and guardrails are documented on their own page:
[`mnemom agent`](/gateway/agent).

```bash
mnemom agent                    # prompts for a session name + optional goal
...
List agents across the orgs you belong to.

```bash
mnemom agent list               # every org you belong to
mnemom agent list --org <id>    # one org
```
