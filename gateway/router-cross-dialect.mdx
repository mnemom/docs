...
Provider-dialect credentials such as Anthropic's `x-api-key`, OpenAI's `Authorization` header, or Gemini's `x-goog-api-key` do **not** authorize `/router`. Unlike the passthrough provider doors, the router initially accepts Mnemom API keys only; browser sessions and other bearer credentials are not router credentials.
