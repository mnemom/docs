...
| Header | Meaning |
|---|---|
| `X-Mnemom-Verdict` | Structured, always present: `front=<v>; autonomy=<v>; integrity=<v>; back=<v>`, one entry per phase — Safe House front door, action-policing (policy + bounded actions), AIP integrity, Safe House back door. Each `<v>` is one of `pass`, `observed`, `nudged`, `enforced`, `unverified`. Absent phases default to `pass`. |
