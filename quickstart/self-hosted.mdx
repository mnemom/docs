...
description: "Run the Mnemom Gateway on your own infrastructure with Docker or Kubernetes"
sidebarTitle: "Self-Hosted"
icon: "server"
---

Run the Mnemom Gateway on your own infrastructure for full data residency control. Prompt and response content is never sent to Mnemom's cloud, though prompts are forwarded to your configured LLM providers — see [Data residency](#data-residency) for exact traffic boundaries. The self-hosted gateway is a Node.js adapter that runs the same code as Mnemom's managed gateway service — identical behavior, your infrastructure.
...
<Steps>
  ...
  <Step title="Configure environment">
    ...
    <Warning>
    `SUPABASE_URL` must be a REST-capable endpoint — a hosted Supabase project, or a self-hosted stack that exposes Supabase's PostgREST API — not a raw Postgres connection string. The gateway talks to it over HTTP (`GET <SUPABASE_URL>/rest/v1/...`), so pointing it at a bare `postgres://` host will make every data call, and the `/health/ready` Supabase check, fail. The bundled `postgres` container in this compose file provides storage for your own Supabase/PostgREST deployment to sit in front of — it is not itself a REST endpoint.
    </Warning>
  ...
  <Step title="Verify health">
    ...
    `status` is `"degraded"` (HTTP 503) and the individual check is `{ "ok": false }` whenever Redis, `SUPABASE_URL`, or the license can't be reached/validated — most commonly `supabase` failing because `SUPABASE_URL` isn't a REST-capable endpoint yet (see the warning above).
  </Step>
...
<Steps>
  ...
  <Step title="Create a Kubernetes Secret">
    ...
    These are the keys the chart's default `values.yaml` (`secrets.values`) expects; add others only if your own values override adds them.
  </Step>
...
The gateway process itself validates only these three — anything else missing fails it closed at boot with an `EnvValidationError` naming the missing variable(s):

| Variable | Description |
|---|---|
| `SUPABASE_URL` | A REST-capable endpoint — hosted Supabase project (`https://<ref>.supabase.co`) or a self-hosted stack exposing PostgREST. Not a raw Postgres connection string. |
| `SUPABASE_SECRET_KEY` | Supabase service-role key |
| `ANTHROPIC_API_KEY` | Anthropic API key (required for AIP analysis) |

`REDIS_PASSWORD` and `POSTGRES_PASSWORD` are required at a different layer — the Docker Compose file itself refuses to start those containers without them (see the compose stack above). `MNEMOM_LICENSE_JWT` is your Enterprise license from [mnemom.ai/dashboard](https://mnemom.ai/dashboard); the gateway still starts without it but `/health/ready`'s `license` check reports it as an unlicensed deployment. `SUPABASE_JWT_SECRET` and `INTERNAL_API_KEY` are not read by the gateway/observer roles this compose stack runs — leave them unset unless you're also self-hosting a role that documents needing them.
...
| `/health/ready` | Readiness probe | Checks Redis, the Supabase/PostgREST endpoint, and license validity |
...
<Warning>
`/metrics` is shut by default — set `METRICS_SCRAPE_KEY` or the endpoint answers every scrape as if the path didn't exist (no 401, just a generic not-found body, so a misconfigured scraper won't even show up as a permission error). Present it as `X-Metrics-Key: <key>` or `Authorization: Bearer <key>`; the latter is what lets a Prometheus Operator `ServiceMonitor` attach it via `bearerTokenSecret`.
</Warning>
