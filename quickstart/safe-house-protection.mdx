...
Start with `observe` mode. This runs full threat analysis with zero latency impact, so you can see what Safe House would catch before committing to blocking. Safe House configuration lives on the agent's **protection card** ([ADR-039](https://github.com/mnemom/scale/blob/main/decisions/ADR-039-unified-two-card-schema.md)) — `mode` is the top-level master switch; `screen_surfaces` decides which surfaces the detector pipeline inspects.
...
Look for Safe House state in the response. Per [ADR-042](https://github.com/mnemom/scale/blob/main/decisions/ADR-042-header-rationalization.md), the legacy `X-Safe-House-*` headers were retired in favor of the unified `X-Mnemom-Verdict` four-checkpoint structure (see [Headers reference](/api-reference/headers)):
...
This time the response is a 422 with a quarantine ID (the [canonical errors contract](/api-reference/errors#a-22-safe-house-quarantine)):
