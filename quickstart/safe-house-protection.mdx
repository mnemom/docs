...
From here on, a request that hands tool results back to the model is screened **again inside that same request** — once per tool result, on its own surface, before the body is forwarded upstream. Under `nudge` and `enforce` the gateway takes the first three tool results the body carries. This is a front-door check; nothing waits for the agent's next turn.

The enforcement shape differs from Step 5. A flagged *message* fails the request with a 422 or 403. A flagged *tool result* does not fail the request — it is removed from the body:
