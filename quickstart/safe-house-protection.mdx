...
From here on, a request that hands tool results back to the model is screened **again inside that same request** — once per tool result, on its own surface, before the body is forwarded upstream. This is a front-door check; nothing waits for the agent's next turn.
