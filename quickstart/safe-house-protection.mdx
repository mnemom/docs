...
Start with `observe` mode. This runs full threat analysis with zero latency impact, so you can see what Safe House would catch before committing to blocking. Safe House configuration lives on the agent's **protection card** — `mode` is the top-level master switch; `screen_surfaces` decides which surfaces the detector pipeline inspects, and each surface has its own timing: `incoming` is screened when the request arrives, `tool_responses` turn-internally (once per tool result, inline, before the request is forwarded upstream), and `outgoing` at the back door on the way out.

The card below leaves `tool_responses` off to keep the first run simple. Turning it on adds one awaited screen per tool result in the request path, and in `enforce` it lets Safe House withhold a single tool result mid-turn — replacing its content with a quarantine notice while the rest of the turn proceeds — instead of failing the whole request. See [Turn-internal tool-result screening](/concepts/safe-house#turn-internal-tool-result-screening).
...
`front` is a rollup of every front-door screen on the request, not a single evaluation — once `tool_responses` is enabled, one turn produces one screen for the message plus one per tool result, and they all fold into that single `front=` value.
</Note>
