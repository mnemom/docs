...
<Steps>
  ...
  <Step title="Claim your agent">
    ...
    ```bash
    mnemom agents claim mnm-a1b2c3d4e5f6 --name my-agent --key $ANTHROPIC_API_KEY
    ```

    Replace `mnm-a1b2c3d4e5f6` with the actual id from your `X-Mnemom-Agent` header.

    - `--name` **must** match the `x-mnemom-agent` value you sent in the gateway call, or claim returns `403 invalid_hash_proof`.
