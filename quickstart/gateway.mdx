...
<Tip>
  Looking to launch a **governed coding agent** instead of wiring up your own application's API calls? See [`mnemom agent`](/gateway/agent) — install once, run `mnemom agent`, and it launches Claude Code through the gateway with an agent set up for you.
</Tip>
...
<Steps>
  ...
  <Step title="Authenticate">
    ...
    This opens a browser-based login flow and stores your auth token in `~/.mnemom/auth.json`. On a machine with no local browser (SSH, a container), use `mnemom login --no-browser` instead — see [CLI Reference](/gateway/cli#authentication).
  ...
  <Step title="What you'll want to read on the way back">
    ...
    | `X-Mnemom-Verdict` | Always (gateway) | Structured `front=…; autonomy=…; integrity=…; back=…` with each value in `{pass \| observed \| nudged \| enforced \| unverified}`. Parse it; the four-checkpoint state tells you what Safe House observed (front+back), what CLPI did on tool calls (autonomy), and what AIP did on reasoning (integrity). `front` is a rollup over every inbound surface the request carried — the message *and* each tool result screened inside that same request — so `front=enforced` on a `200` means a tool result was withheld or decorated before the model saw it. See [When the front door runs](/concepts/safe-house#when-the-front-door-runs). `unverified` is integrity-only: the analyzer failed, so no trustworthy verdict exists — `enforce` withholds the response (fail-closed, still 2xx), `observe`/`nudge` forward it and record the unverified state. Never reported as `pass`. |
    ...
    See the [Headers reference](/api-reference/headers) for the full canonical set + per-language parsers, and the [Errors reference](/api-reference/errors) for the verdict-to-status mapping.
  </Step>
  ...
  <Step title="Claim your agent">
    ...
    - The agent lands in your active org (set with `mnemom org use <slug>`), or your personal org if you haven't set one; pass `--org <slug>` to claim into a specific shared org instead.
  ...
  <Step title="Check status">
    ...
    This prints an authentication / gateway / API connectivity checklist, then your agent ID, the gateway URL, and a dashboard link, followed by a trace summary once the agent has traffic.
  </Step>
  ...
  <Step title="View traces">
    ...
    Each trace prints as its own block — timestamp, action, type, and (when present) the reasoning excerpt and any policy violations. Use `mnemom logs --agent my-agent --limit 20` to show more entries.
  </Step>

  <Step title="Check activity">
    View your agent's AAP behavioral activity — total traces, how many verified clean, and any violations:
    ...
    <Note>
    `mnemom integrity` is a deprecated alias for this same command — `mnemom activity` is the current name. Despite the name, this surfaces AAP trace verification, not the AIP per-turn [integrity checkpoints](/concepts/integrity-checkpoints); AIP checkpoint data is not yet exposed through the CLI.
    </Note>
  </Step>
...
|----------|--------|----------------------|-------------|
| Anthropic | Claude Fable 5.1, Claude Fable 5, Claude Opus 5.5, Claude Opus 5, Claude Sonnet 5, Claude Opus 4.8, Claude Opus 4.7, Claude Sonnet 4.6, Claude Haiku 4.5 | Full (thinking blocks analyzed directly) | `x-api-key` |
| OpenAI | GPT-6 Astra, GPT-5.6 Sol, GPT-5.6 Terra, GPT-5.6 Luna, GPT-5, GPT-5 Codex, o3, o3-mini | Not available through the gateway today | `Authorization: Bearer` |
| Gemini | Gemini 3.8 Flash, Gemini 3.5 Flash-Lite, Gemini 2.5 Pro, Gemini 2.5 Flash | Full (thought parts analyzed directly) | `x-goog-api-key` |
...
|-----------------|-------------|--------|
| Anthropic reasoning models (Opus, Sonnet, Fable) | Full | Thinking blocks analyzed directly |
| Gemini 2.5/3 with thinking | Full | Thought parts analyzed directly |
| Non-reasoning models | Tracing only | Synthetic `clear` verdict, no analysis-LLM call |
| OpenAI (all models, including the o-series) | Not available | The gateway's `/openai` door proxies Chat Completions, which never returns reasoning content — see [Provider Support](/concepts/provider-support#2-openai--no-thinking-trace-inspection-through-the-gateway-today) for the full disclosure |
...
| `enforce` | Gates same-turn on both streaming and non-streaming requests. The request still completes with a `2xx` status — the response body is replaced with an agent-voice intervention message and `X-Mnemom-Verdict` reports `integrity=enforced`. On streaming this adds latency because the response is evaluated before it is delivered. |
