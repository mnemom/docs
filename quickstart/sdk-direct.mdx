...
description: "Integrate alignment verification, integrity checking, and policy governance directly into your application"
...
<Note>
`--legacy-peer-deps` is needed on npm 7+ while `@mnemom/agent-integrity-protocol` 1.3.0 still declares an optional peer on the 1.x alignment-protocol package; the two SDKs work together at 2.0.0 / 1.3.0.
</Note>
...
<Steps>
  <Step title="Define an Alignment Card">
    ...
    <Note>
    **Field names changed in the unified card shape (AAP 2.0.0).** `aap_version` is now `card_version` (date-anchored); `autonomy_envelope`/`AutonomyEnvelope` is now `autonomy`/`Autonomy`; `audit_commitment`/`AuditCommitment` is now `audit`/`Audit`; `autonomy_mode` and `integrity_mode` are new top-level fields; `principal.identifier` is required whenever `principal.type` isn't `unspecified`; and `audit.queryable: true` requires `audit.query_endpoint`. See [migrating from the legacy 0.5.0 card shape](/protocols/aap/specification#410-migrating-from-the-legacy-050-card-shape) for the full mapping.
    </Note>
  ...
  <Step title="Add integrity checkpoints">
    ...
    **AIP defaults to fail-open mode.** If the analysis LLM is unreachable, integrity checks will silently pass. For production deployments handling sensitive operations, set `failure_policy: { mode: "fail_closed" }` in your AIP configuration.
    ...
    A client does the extraction, the LLM call, and the checkpoint/window bookkeeping for you — hand it the raw provider response body, not a pre-extracted thinking string. The `card` here is AIP's own minimal card shape (`card_id`, `values`, `autonomy_envelope`) — a small subset of fields, kept separate from the full AAP `AlignmentCard` you built in Step 1, since `@mnemom/agent-alignment-protocol` is only an optional peer dependency for AIP.
    ...
    <Tip>
    `client.check()` runs the full pipeline every call: extract the thinking block for the detected provider, call the analysis LLM, parse the verdict, update the sliding window, and detect drift. There is no separate low-level "just parse this analysis response" function in the public API — `check()` is the one entry point.
    </Tip>
