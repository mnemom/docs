...
|--------|----------|-------------|
| `POST` | [`/v1/agents`](/api-reference/endpoint/post-agents) | Register a new agent with name, hash proof, and optional card/policy |
...
| `DELETE` | [`/v1/agents/:id`](/api-reference/endpoint/delete-agents-agent-id) | Deactivate an agent (soft-delete, preserves traces) |

When registering an agent, you must provide a `hash_proof` -- the SHA-256 hex digest of your API key combined with the agent name (`SHA256(apiKey + '|' + agentName)`). This proves ownership of the API key without transmitting it.

```bash
# Register a new agent with an Alignment Card
...
The response includes the `agent_hash` (the gateway-computed identity), `card_id` (if a card was attached), and `policy_created` (if a policy was attached). If an agent with the same hash already exists, the API returns `409 Conflict`.
