...
1. **Static validation** -- `mnemom policy validate` checks schema correctness. It is fast, offline, and makes no API calls.
2. **Policy evaluation** -- `mnemom policy evaluate` checks the policy against live agent data. It requires an API key and network access.
  ...
  validate-policy:
    ...
    steps:
      ...
      - name: Validate policy
        run: mnemom policy validate policy.yaml
```
  ...
  evaluate-policy:
    ...
    steps:
      ...
      - name: Evaluate policy
        run: mnemom policy evaluate --agent my-agent --format json
```
...
evaluate-policy:
  ...
  script:
    - npm install -g @mnemom/smoltbot
    - mnemom policy evaluate --agent my-agent
  only:
    - main
```
...
```bash
mnemom policy evaluate --agent my-agent --format json
```
...
<Tip>
  Pipe the JSON output to `jq` for extracting specific fields in downstream pipeline steps. For example, `mnemom policy evaluate --agent my-agent --format json | jq '.coverage.coverage_pct'` extracts just the coverage percentage.
</Tip>
...
jobs:
  policy-gate:
    ...
    steps:
      ...
      - run: npm install -g @mnemom/smoltbot
      - run: mnemom policy validate policy.yaml
      - run: mnemom policy evaluate --agent my-agent
        env:
          MNEMOM_API_KEY: ${{ secrets.MNEMOM_API_KEY }}
...
- [Policy CLI Reference](/gateway/policy-cli) -- CLI command details for `validate`, `evaluate`, `test`, and `publish`
