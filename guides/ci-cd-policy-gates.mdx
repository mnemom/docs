...
1. **Static validation** -- `mnemom card validate` checks schema correctness. Without `--agent` (or `MNEMOM_AGENT`) set it makes no API calls; pass `--agent <name>` to validate server-side against your org/platform floor instead.
2. **Card evaluation** -- `mnemom card evaluate` checks the card's policy against a set of tools you supply. It always runs entirely locally against the embedded policy engine -- no API key, and no agent lookup, either way.
...
Card evaluation goes beyond schema validation. It evaluates your card's policy against a tool list you provide -- checking capability mapping coverage, verifying tool permissions, and scoring the card against that tool usage. It runs entirely locally against the embedded policy engine, so it needs no `MNEMOM_API_KEY` and makes no network call.

<Tip>
  Because `card evaluate` is fully local, it only checks the tools you pass via `--tools`/`--tool-manifest`. It does not read your agent's actual traffic -- for that, evaluate against real recent traces with `POST /v1/policies/evaluate/historical` (see [Policy Management](/guides/policy-management#historical-evaluation)).
</Tip>
...
This workflow runs on pushes to `main`, making it a pre-deploy gate.
...
evaluate-card:
  ...
  script:
    - npm install -g @mnemom/mnemom
    - mnemom card evaluate card.yaml --tools mcp__browser__navigate,mcp__slack__post_message --strict
  only:
    - main
```
...
`card validate` and `card evaluate` need no API key for the local checks above, but the reputation gate (and a server-authoritative `card validate --agent <name>`, if you use one) do. In GitHub, go to Settings > Secrets and variables > Actions, then add `MNEMOM_API_KEY` with your API key. In GitLab, go to Settings > CI/CD > Variables and add it there with the "Masked" option enabled.
