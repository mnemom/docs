...
If you're coming from the pre-UC `sh_configs` JSON blob, start with the [migration guide](/guides/migrating-to-unified-cards); the automatic UC-3 migration already composed your old configs into protection cards.
...
| Field | Composition |
|-------|-------------|
| `mode` | **Strictest wins** (`enforce` > `nudge` > `observe`). Agent can go stricter, not looser. |
| `thresholds.*` | **Floor + override**: org threshold is the ceiling (loosest acceptable); agent can set lower (stricter). |
| `screen_surfaces.*` | **True wins**: any scope can require scanning a surface; agents cannot turn off scanning the org requires. |
| `trusted_sources.domains` | **Platform intersection, org+agent union**: platform trust list caps the maximum; downstream scopes can only add from that cap. |
...
thresholds:
  injection_score: 0.55            # stricter than the 0.70 platform default
  exfiltration_score: 0.70
  canary_match: 1.0
  dlp_confidence: 0.60
...
thresholds:
  injection_score: 0.50             # lower = more sensitive (more events logged)
  exfiltration_score: 0.70
```

Review the event stream for 7-14 days. Adjust thresholds based on false-positive rate. Promote to `nudge` or `enforce` when stable.
