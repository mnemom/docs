...
By default this is server-authoritative: it runs the full schema validator and an inline
composition against the live platform/org templates, so you see the actual canonical output
without writing anything. Pass `--offline` to validate against the schema only, with no network
...
<Warning>
`tool_responses: false` is the only thing standing between an injected tool result and the model. With it off, nothing downstream re-screens that content — AIP reads the model's *reasoning* afterwards, on the response path, and by then the payload has been read. Trade the latency knowingly. See [When the front door runs](/concepts/safe-house#when-the-front-door-runs).
</Warning>
...
Safe House verdicts emit `sh.evaluation.warn` / `sh.evaluation.quarantine` / `sh.evaluation.block`
webhook events if your org has a webhook endpoint subscribed to them. See the [Webhook Event
Catalog](/api-reference/webhook-events#safe-house) for the exact payload shape, and [Webhook
Notifications](/guides/webhooks) for creating an endpoint.
...
There's no first-class rollback endpoint for protection cards, and no amendment-history read
endpoint either (`GET /v1/agents/{agent_id}/card-amendments` tracks alignment-card edits only —
...
- [Webhook Notifications](/guides/webhooks) — alerting integration
