...
> **us-2 cutover (2026-08-04):** On 2026-08-04 Mnemom's **managed** production cut over onto the
> Azure **us-2 cell**: `ca-*-us2` container apps in resource group `rg-mnemom-cell-us2-eastus2`,
> fronted by Cloudflare, with identity on Zitadel `id-us2.mnemom.ai`. Managed compute moved off
> Cloudflare Workers onto Azure container apps. Mnemom retired the legacy Supabase project as the
> managed datastore. Cloudflare remains as the fronting CDN/edge. References to "Cloudflare Workers"
> compute and "Supabase" storage below describe the **pre-cutover managed** topology. The
> **self-hosted** stack still ships on Cloudflare Workers + Supabase — see the [self-hosted quickstart](/quickstart/self-hosted).
