...
**Mnemom Gateway** is the operational layer that runs both protocols. It intercepts LLM API traffic, extracts thinking blocks for AIP analysis, generates AP-Traces for AAP verification, and enforces configurable [enforcement modes](/gateway/enforcement) based on the combined results.
...
|-----------|-------------|
| **AP-Traces** | Structured decision records: what action the agent took, which values were applied, what alternatives were considered, and whether escalation was evaluated |
| **Integrity Checkpoints** | Verdict (`clear`, `review_needed`, `boundary_violation`), concern categories, severity, reasoning summary, and a SHA-256 hash of the thinking block |
| **Alignment Cards** | Agent identity declarations: values, autonomy envelope, escalation triggers, audit commitment |
| **Drift Alerts** | Alerts generated when behavioral similarity drops below threshold over sustained turns (AAP) or when integrity patterns degrade within a session window (AIP) |
| **Enforcement Records** | Records of enforcement actions taken (observe, nudge, enforce) in response to violations |

All of the above are stored encrypted at rest in Mnemom's database.
...
| Cookie | Shape |
|--------|-------|
| `__Host-mnemom_session` | HttpOnly, Secure, Path=/, SameSite=Lax. The `__Host-` prefix means the browser refuses to accept it without `Secure`+`Path=/` and forbids a `Domain` attribute — the cookie is host-locked to `api.mnemom.ai`, so no sibling subdomain can plant or overwrite it. |

The cookie value is an AES-256-GCM-encrypted blob of the session's access/refresh tokens. Tokens are never exposed to JavaScript (HttpOnly) and are transparently refreshed + rotated server-side when the access token nears expiry.

MFA step-up is carried through a short-lived `__Host-mnemom_mfa_pending` cookie between `POST /v1/auth/sign-in` and `POST /v1/auth/mfa/verify`; the pre-MFA access token never reaches JS. SSO uses a single-use `mnemom_sso_initiator` cookie bound to signed state.
...
The JWT is issued on the user's behalf and contains identity, organization membership, and role. The CLI obtains its token via a one-time browser → localhost handoff at login time (mediated by `POST /v1/auth/cli-exchange`, which itself requires a valid session cookie). Tokens are verified on every request via JWKS.
...
Role-based access is enforced at the API and database layers via row-level security policies. Organization-scoped API keys inherit the permissions of the role they were created under.
...
- The Mnemom gateway
- The Mnemom API (`api.mnemom.ai`)
- Webhook delivery (signed with HMAC-SHA256 — see [Webhook Notifications](/guides/webhooks))
- Cross-protocol communication between AAP and AIP components

Certificate management is handled automatically at the edge.

### At rest

All persisted data — AP-Traces, Integrity Checkpoints, Alignment Cards, drift alerts, enforcement records, and user/organization data — is stored in an encrypted-at-rest database.
...
- **Model inference correctness.** The zero-knowledge proof layer proves that the deterministic verdict-derivation rules were applied honestly to the model's output. It does **not** prove the model inference itself was run correctly. Proving model inference at current scale is prohibitively expensive.
...
AIP requires models that expose thinking content. Current provider support:

| Provider | Thinking source |
|----------|----------------|
| **Anthropic** | Native `thinking` content blocks |
| **OpenAI** | `reasoning_content` field |
| **Google Gemini** | Thought parts (`generationConfig.thinkingConfig.includeThoughts`) |

Each Integrity Checkpoint carries an `extraction_confidence` field reflecting how reliably the thinking content was extracted for that turn. Models without any exposed thinking content cannot be meaningfully analyzed by AIP — this is a known gap: models with fully opaque reasoning are invisible to AIP.
...
1. **Trust the signing key.** If the Ed25519 private key is compromised, an attacker can forge signatures on arbitrary checkpoints. Signing keys are held in Mnemom's server-side secret store and are never exposed to client code. Key rotation is supported via the `key_id` field in certificates.
...
The certificate verification endpoints are public and unauthenticated. Anyone can:
...
An agent's current Merkle root (`GET /v1/agents/{id}/merkle-root`) and its ZK proof count (`GET /v1/agents/{id}/zk-proof-count`) are per-agent reads, so they require an authenticated member of the agent's org; an anonymous request gets `401`. A certificate already carries the Merkle root it was proven against (`proofs.merkle.root`), so a third party can still walk the inclusion proof offline without calling either endpoint.
...
<Warning>
Do not open public GitHub issues for security vulnerabilities. Use the Security Advisories feature (Security tab on each repository) to report vulnerabilities privately, or email security@mnemom.ai. We acknowledge within 3 business days, confirm reproduction within 14 days, and target a fix or mitigation within 90 days of acknowledgment (sooner once a fix ships and customers are protected). See [SLA and Incident Response](/guides/sla-and-incident-response#how-customers-reach-us) for the full policy.
</Warning>
