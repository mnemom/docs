...
> Cloudflare-fronted; identity on Zitadel `id-us2.mnemom.ai`). Managed compute moved off Cloudflare
> Workers onto Azure container apps, and the legacy Supabase project was retired as the managed
> datastore; Cloudflare remains as the fronting CDN/edge. References to "Cloudflare Workers" compute
> and "Supabase" storage below describe the **pre-cutover managed** topology. (The **self-hosted**
> stack still ships on Cloudflare Workers + Supabase — see the [self-hosted quickstart](/quickstart/self-hosted).)
