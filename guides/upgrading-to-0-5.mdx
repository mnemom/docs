...
<Note>
**Historical migration record.** AAP is now at 2.0.0 (AIP is at 1.3.0). New integrations should follow [Upgrading to AAP 2.0.0 & AIP](/guides/upgrading-to-1-0). This page is kept for customers still on `0.1.0` cards and for the YAML authoring and Trust Edges context it introduced.
</Note>
...
`weight` is a number from 0 to 1 (default 0.5 if omitted). `reason` is constrained to `team_membership`, `manual`, or `org_co_membership`. A higher weight gives more influence to the target agent's reputation in coherence calculations.
