...
<Warning>
**AAP is now at 1.0.0.** This page documents the historical `0.1.0 → 0.5.0` upgrade for customers still on cards at that range. New integrations should target [AAP 1.0.0](https://github.com/mnemom/aap/releases) directly and skip this intermediate upgrade.

The migration path from 0.5.0 → 1.0.0 is the same shape as described here — update the `aap_version` string and rebuild. 1.0.0 is fully backward-compatible with 0.5.0 cards; no schema changes are required. Replace `"0.5.0"` with `"1.0.0"` wherever you see it in the examples below.
</Warning>
