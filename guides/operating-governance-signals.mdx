...
Every state transition records `acknowledged_actor_role` (who acted, in what capacity) per [ADR-046](https://github.com/mnemom/scale/blob/main/decisions/ADR-046-audit-actor-model.md). `resolution_status` + `action_taken` provide the prose audit trail. The platform's `governance_audit_log` table captures every governance mutation for full SOC2-shaped reconstruction.
