...
`GET /v1/orgs/:org_id/governance/coverage?window_days=30` returns a per-(source, severity) aggregate over the window (`window_days` is 1-90, default 30):

```bash
curl -H "Authorization: Bearer $TOKEN" \
  "https://api.mnemom.ai/v1/orgs/org-id/governance/coverage?window_days=30" | jq
```
