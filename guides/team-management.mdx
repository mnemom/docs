...
<Info>
Team operations require your org's plan to include the `team_reputation` capability. If it isn't enabled, team endpoints return `403 feature_gated` with an `upgrade_url` pointing at [Pricing](/pricing/overview).
</Info>
...
Create a team, add members, and derive a card in three API calls. There is no `@mnemom/teams` package — use the REST API directly, or `@mnemom/sdk`'s generic HTTP client:
...
# 2. Derive a team alignment card from member cards (at least 2 members
#    need active alignment cards first)
derived = httpx.post(f"{API_BASE}/v1/teams/{team_id}/card/derive", headers=HEADERS)
print(f"Derived card: {derived.json()['card_json']}")
```
</CodeGroup>
...
Teams require an organization and a name. You can add up to 50 initial members, or create the team empty and add members afterward — at least 2 members with active alignment cards are needed before you can derive a card (step 3).
...
```
GET https://api.mnemom.ai/v1/teams/{team_id}/badge.svg?variant={variant}&style={style}
```

`style` is `light` (default) or `dark`. Six badge variants are available:
...
| Update team | Org `owner`/`admin`, or a delegated `team_admin` grant on that team |
| Archive team | Org `owner` or `admin` |
| Add/remove members | Org `owner`/`admin`, or a delegated `team_admin` grant on that team |
| Set team card | Org `owner`/`admin`, or a delegated `team_admin` grant on that team |
| Derive team card | Org `owner`/`admin`, or a delegated `team_admin` grant on that team |
...
A `team_admin` grant delegates day-to-day management of one team without full org `admin` — manage grants with `mnemom team admin grant`/`revoke`/`list` or `GET/POST /v1/teams/{team_id}/admins`.
