...
|---|---|---|---|
| **GV.OC** — Organizational Context | [Alignment Card](/concepts/alignment-cards) declares each agent's `principal`, autonomy bounds, and escalation triggers | PL-1 | The `principal` field identifies the accountable human or org; `autonomy` bounds what the agent may do |
...
| **PR.AA** — Identity Management & Access Control | RBAC: Owner / Admin / Member roles enforced at the API layer via row-level authorization policy in Mnemom's managed database; org-scoped API keys inherit the creating role's permissions | AC-2, AC-3, AC-6 | Role-based access is the primary authorization surface |
...
| **PR.DS** — Data Security | TLS 1.2+ on all endpoints — Mnemom Gateway, Mnemom API (`api.mnemom.ai`), webhook delivery, cross-protocol communication | SC-8, SC-13 | TLS is terminated and re-encrypted at Mnemom's edge; certificate management is automatic |
| **PR.DS** — Data Security | Encryption at rest: Mnemom's managed database encrypts all persisted protocol artifacts at rest (AP-Traces, Integrity Checkpoints, Alignment Cards, drift alerts) | SC-28 | No plaintext storage of any protocol artifact |
...
| CSF category | Mnemom mechanism | 800-53 controls | Notes |
|---|---|---|---|
| **DE.CM** — Continuous Monitoring | AIP per-turn integrity checkpoints: an analysis LLM evaluates agent thinking blocks in the ~2-second window between reasoning and action (in `enforce` the response is gated before delivery; in `observe`/`nudge` the verdict is recorded post-hoc); produces `clear` / `review_needed` / `boundary_violation` verdicts | SI-4, CA-7 | Anthropic native thinking blocks (confidence 1.0), OpenAI `reasoning_content` (0.9), Google Gemini thinking parts (0.9); models without exposed thinking produce synthetic `clear` verdicts — a documented gap |
...
| CSF category | Mnemom mechanism | 800-53 controls | Notes |
|---|---|---|---|
| **RS.MA** — Incident Management | Four Safe House modes per agent: **Off** (detectors do not run), **Observe** (detectors run, signals logged, no request-path action), **Nudge** (an advisory annotation is attached; the message proceeds), **Enforce** (detectors run synchronously; a match blocks the request) | IR-4 | Mode set per agent via the protection card |
...
| **RS.CO** — Incident Response Reporting | Responsible disclosure: acknowledgment within 3 business days, reproduction confirmed within 14 days, fix or mitigation within 90 days of acknowledgment | IR-6, IR-7 | See [trust.mnemom.ai](https://trust.mnemom.ai) for the full policy |
...
| **RS.MI** — Incident Mitigation | Human escalation triggers: `autonomy.escalation_triggers` in the Alignment Card declare conditions requiring human review before action; enforced by the gateway | IR-4 | Escalation is policy-declared at card design time |
...
| **CP** — Contingency Planning | **Partial** | Trust recovery guide and agent restore/reactivate document recovery procedures. Database backups are managed by Mnemom's cloud infrastructure providers; no published customer-facing RTO/RPO for data recovery — see [CP-9 gap note](#cp-9--system-backup) |
...
| **PE** — Physical and Environmental Protection | **Gap** | Inherited from Mnemom's cloud infrastructure providers; Mnemom publishes no physical security attestations of its own — see [gap note](#pe--physical-and-environmental-protection) |
| **PL** — Planning | **Partial** | Alignment Card principal and autonomy bounds serve as per-agent planning artifacts; no enterprise security plan feature is shipped as a platform control |
...
| **SC** — System and Communications Protection | **Shipped** | TLS 1.2+ on all endpoints, AES-256-GCM session tokens, HMAC-SHA256 webhook authentication, encryption at rest in Mnemom's managed database |
...
Mnemom is a cloud-hosted SaaS. Physical and environmental controls are inherited from Mnemom's cloud infrastructure providers. Mnemom publishes no physical security attestations of its own. Refer to those providers' SOC 2 reports (available under NDA on request) for physical-layer coverage.
