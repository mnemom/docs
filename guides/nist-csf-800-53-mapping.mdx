...
| CSF category | Mnemom mechanism | 800-53 controls | Notes |
|---|---|---|---|
| **ID.AM** — Asset Management | Agent registration: alignment cards carry a stable `agent_id`, `principal`, capability declarations, and bounded action scope; [fleet topology](/api-reference/endpoint/get-agents-topology) queries the full agent graph | CM-8, IA-3 | Every agent is inventoried with identity, declared capabilities, and assigned principal at registration time |
| **ID.AM** — Asset Management | [Posture versioning](/concepts/posture-versioning) and [card amendments](/api-reference/endpoint/get-agents-agent-id-card-amendments) track configuration history with a queryable amendment trail | CM-2, CM-8 | Changes to alignment cards and postures are recorded without data loss |
