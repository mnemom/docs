...
`GET /v1/api-keys/whoami` resolves whichever credential is on the request and echoes back its identity — scopes, the bound user/org, and the `key_id` — without you needing to know it in advance. Useful for confirming a key's scope before wiring it into an integration:

```bash
curl https://api.mnemom.ai/v1/api-keys/whoami \
...
Keys minted before May 2026 carry the legacy two-scope set `["gateway", "api"]` (or, rarer, a single `["api"]`). They continue to work — the auth gate aliases `["gateway", "api"]` to `["gateway", "api:read", "api:write"]` and bare `["api"]` to `["api:read", "api:write"]` at request time, so existing integrations need no changes.
