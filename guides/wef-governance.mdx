...
|-------------|-----------|------|
| Agent function/task | `autonomy.bounded_actions` | String array |
| Function constraints | `autonomy.forbidden_actions` | String array |
...
|-------------|---------------|----------|
| Behavioral predictability | `audit.trace_format` | Structured logging of non-deterministic decisions |
| Non-deterministic monitoring | AIP Integrity Checkpoints | Per-turn analysis of thinking blocks (enforce gates same-turn; observe/nudge post-hoc) |
| Behavioral change over time | AIP `IntegrityDriftAlert` | Cross-session behavioral divergence detection |
| Tamper evidence | `audit.tamper_evidence` | `append_only`, `signed`, or `merkle` trail integrity |
...
|-------------|-----------|----------|
| Autonomy level | `autonomy` (composite) | Complete autonomy specification |
| What agent can do independently | `autonomy.bounded_actions` | Permitted autonomous actions |
| When agent must stop and ask | `autonomy.escalation_triggers` | Condition-based escalation rules |
| Financial limits on autonomy | `autonomy.max_autonomous_value` | Currency-denominated ceiling |
| Who to escalate to | `principal.escalation_contact` | Endpoint for escalation notifications |
| Per-turn calibration | AIP `recommended_action` | `continue`, `log_and_continue`, `pause_for_review`, `deny_and_escalate` |
...
| WEF Concept | AAP Field | Function |
|-------------|-----------|----------|
| System permissions | `autonomy.bounded_actions` | What the agent is permitted to do |
| Permission boundaries | `autonomy.forbidden_actions` | Hard limits regardless of context |
| Data access scope | `autonomy.escalation_triggers` | Conditions that constrain data access |
...
|----------------------|-----------|----------------|
| Structured records | `audit.trace_format` | `"ap-trace-v1"` — standardized, schema-validated |
| Retention policy | `audit.retention_days` | Explicit retention period |
| Queryable logs | `audit.queryable` + `query_endpoint` | API-accessible trace history |
| Tamper resistance | `audit.tamper_evidence` | `append_only`, `signed`, or `merkle` |
...
| **Legal and compliance** | Data protection impact assessments; privacy and regulation compliance | `values.declared` encodes compliance values; `extensions` namespace for regulatory metadata; `audit` enables DPIA evidence |
