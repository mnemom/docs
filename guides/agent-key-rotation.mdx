...
Verify binding also works **without a Mnemom credential** — it's a public, anti-impersonation check anyone holding `(agent_id, key)` can run. An anonymous or cross-org caller gets `{ "bound": true, "caller": "anonymous" }` (no `key_prefix`); only a member of the agent's org sees the prefix.
