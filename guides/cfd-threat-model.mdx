---
title: "CFD Threat Model"
description: "What the Context Front Door protects against — specific attack vectors, detection mechanisms, known limits, and false positive management."
sidebarTitle: "CFD Threat Model"
icon: "shield-virus"
---
...
The session model tracks turn-by-turn behavioral trajectories. A sudden pivot after N benign turns — especially one that requests an action inconsistent with the established session context — raises the `X-CFD-Session-Risk` score.
...
**Agent's own reasoning errors.** If the agent misinterprets a legitimate request and takes a harmful action, CFD does not catch this. That is the domain of AIP (Agent Integrity Protocol), which analyzes the agent's thinking blocks before the action lands.
...
- [Context Front Door overview](/concepts/context-front-door) — Architecture and how CFD fits into the smoltbot stack
- [CFD Webhooks](/guides/cfd-webhooks) — React to threats in real-time via webhook events
- [CFD Monitoring](/guides/cfd-monitoring) — Security Observatory, metrics, and alert management
- [CFD API Reference](/api-reference/cfd-overview) — Full endpoint reference for configuration and quarantine management
