...
description: "Add trust signals to your README, website, Agent Card, or documentation with dynamic SVG badges showing your agent's Mnemom Trust Rating™."
...
Badges are served as SVGs from a public endpoint. No authentication required.
...
```
GET https://api.mnemom.ai/v1/reputation/{agent_id}/badge.svg?variant={variant}&style={style}
```
...
| `style` | query | No | `light` (default) or `dark` -- changes only the label background color |

**Response:** `image/svg+xml` with `Cache-Control: public, max-age=3600, s-maxage=3600` (1-hour cache).

---

## Badge variants

Every variant except `score_tier` uses the label "Trust Score"; `score_tier` alone uses "Mnemom Trust". The tier name shown by `score_tier` comes from the canonical grade/tier table (e.g. `BBB` → `Developing`) -- see [Score range and grades](/concepts/reputation-scores#score-range-and-grades).

### `grade` (default)

Displays the letter grade only.
...
```
https://api.mnemom.ai/v1/reputation/agent-xyz/badge.svg?variant=grade
```

### `score`

Displays the numeric score only.
...
Displays the grade followed by the numeric score.
...
```
https://api.mnemom.ai/v1/reputation/agent-xyz/badge.svg?variant=score_grade
```

### `score_tier`

Displays the numeric score with the human-readable tier label, under the "Mnemom Trust" label instead of "Trust Score".

```
[ Mnemom Trust | 782 Developing ]
```
...
Displays the grade and score with a directional trend arrow showing 30-day movement.
...
Displays only the letter grade, no label -- **not** the numeric score.

```
[ BBB ]
```

Best for: tight layouts, inline badges, status bars.
...
Agents that have not yet reached the 50-checkpoint minimum display a progress badge instead of a score, regardless of the requested `variant`:

```
[ Trust Score | Building 23/50 ]
```

This badge automatically transitions to the requested variant once the agent becomes eligible.

### Private agents

Agents whose reputation `visibility` is set to `private` display a generic badge rather than leaking a score:
...
Agents with no reputation record at all (never registered, or not yet computed) display a neutral "Not Rated" badge instead of a broken image:

```
[ Trust Score | Not Rated ]
```

This ensures badge embeds degrade gracefully.
...
<Info>
The "Not Rated", "Building", and "Private" badges are all returned with the same `Cache-Control` header as scored badges. Once an agent is registered, becomes eligible, or is made public, the badge automatically updates.
</Info>
...
|-------|-----|-------------|
| CDN / browser cache | 60 minutes | `Cache-Control: public, max-age=3600, s-maxage=3600` on the SVG response |
| Badge-generation cache | 5 minutes | Internal KV cache of the generated SVG, keyed by agent + variant + style |
...
The `style` query parameter (`light` or `dark`) changes only the **label** background -- medium gray for `light` (the default), darker gray for `dark`. The **value** side is always colored by grade, regardless of `style`:
...
Team badges use the same caching behavior as agent badges (1-hour CDN/browser cache, 5-minute internal regeneration cache).

---

## GitHub Action: CI/CD reputation gates

[`mnemom/reputation-check`](https://github.com/mnemom/reputation-check) is a GitHub Action that gates a workflow step on an agent's minimum reputation score. See the action's repository for current inputs, outputs, and a usage example -- it is a separate open-source project from this API, so its exact interface is versioned independently.
...
The embed modal supports all badge variants and four formats (Markdown, HTML, React, A2A).
...
- [Scoring Methodology](/concepts/reputation-scores) -- Full technical specification
