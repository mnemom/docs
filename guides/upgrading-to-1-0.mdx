...
<Tabs>
  ...
  <Tab title="Via YAML / JSON file">
    If you version-control your card as a local file, a version-number bump alone still fails `mnemom card validate` — the platform expects the **unified / ADR-039 field structure**, which differs from the 0.x card layout. Update your file to the unified shape:
