...
|-------|--------------|----------|
| `sh.evaluation.warn` | Safe House flags a screened surface above the warn threshold but below block | Informational |
| `sh.evaluation.quarantine` | Safe House quarantines a screened surface for human review | High |
...
An `sh.evaluation.*` event is emitted per **screened surface**, not per turn. One request can emit several — one for the inbound message, and one for each tool result screened inside that same request before the model sees it. Do not assume one evaluation event per agent turn. See [When the front door runs](/concepts/safe-house#when-the-front-door-runs).
