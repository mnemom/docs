...
- The gateway respects the flag: when serving a `needs_recompose` canonical, it bypasses its 5-minute cache so changes are picked up as soon as recompose finishes.

To see whether a given agent's recompose has landed, re-fetch its canonical card and check `_composition.composed_at`:
...
A `composed_at` at or after your template publish time means that agent has recomposed. The `X-Card-Version` response header also increments on every recompose, so a simple poll loop can watch for it to change.
...
`exempt_section` must be one of `autonomy.forbidden_actions`, `enforcement.forbidden_tools`, or `autonomy.max_autonomous_value`. `granted_by` is not a request field — the API stamps it from your authenticated identity, so you can't grant an exemption on someone else's behalf.

### Rules

| Rule | Why |
|------|-----|
| Exemption expires (default 90 days from grant). Set `expires_at: null` for permanent — omit `expires_at` for the default. | Exemptions are narrow, contextual, and time-bounded by design. |
| `reason` is required, 20–500 characters. | Audit trail is the whole point. |
| Only the three sections above can be exempted — `conscience.values` (including `BOUNDARY`-typed entries) is not an exemptable section at all. | Inviolable commitments are inviolable. |
| Granting or revoking an exemption requires org `owner` or `admin`. | Waiving an org floor is an org-admin decision. |
