...
```bash
mnemom policy test policy.yaml
```
...
```bash
mnemom policy test policy.yaml --limit 100
```
...
```bash
mnemom policy evaluate --file policy.yaml --agent-id your-agent --min-coverage 90
```
  ...
  validate:
    ...
    steps:
      ...
      - name: Validate policy
        run: npx mnemom policy validate policy.yaml
      - name: Test against traces
        run: npx mnemom policy test policy.yaml --limit 50
        env:
...
<CardGroup cols={2}>
  ...
  <Card title="Test before publishing" icon="flask-vial">
    Always run `mnemom policy test` before `mnemom policy publish`. Testing against historical traces shows you the real-world impact of your policy before it affects live requests.
  </Card>
...
- [Policy CLI Reference](/gateway/policy-cli) -- CLI commands for init, validate, test, publish, and evaluate
