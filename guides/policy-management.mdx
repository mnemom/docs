...
(The AAP 1.0 protocol-level card uses `autonomy_envelope.bounded_actions` — a separate, still-stable surface for external agent interop that is not submitted to `mnemom card validate`.)
