...
| Your Mnemom key / Bearer token | `$MNEMOM_API_KEY` | Authenticates you as the owner — you become `claimed_by` |
...
**AP-Traces are best-effort.** The observer writes AP-Traces when the request's origin metadata survives to the observer layer, but this is not guaranteed for unclaimed agents.
...
An agent already owned by a *different* principal returns `409 agent_cross_tenant` — a resource-state conflict, not a permission failure, since the proof you presented is valid. `hash_proof` lets you claim an unowned agent; it never lets you take over one that already has an owner.
...
1. **Ownership is set.** `claimed_by` is your user/org; every subsequent policy and billing decision flows through your account. The agent's billing pointer is re-stamped to the destination org on every claim and every [move](#idempotency-and-re-homing) — including into an org with no billing account configured — so usage never keeps charging an org the agent has left.
...
|---|---|---|
| `401` | No credentials presented, or the token expired | Add `Authorization: Bearer <token>`, `X-Mnemom-Api-Key: <mnm_key>`, or a valid session cookie |
| `400` — "hash_proof is required" | Body missing `hash_proof` | Compute and include `hash_proof` |
| `403 invalid_hash_proof` | `hash_proof` doesn't match the agent's stored proof (wrong key, wrong name, or malformed hex) | Recompute `SHA256(providerApiKey + '|' + agentName)` and verify the agent name matches exactly |
...
| `409 agent_cross_tenant` | Agent already owned by a different principal | Cannot adopt another owner's agent |
| `404` | `agent_id` doesn't exist, or doesn't exist yet | Verify the `mnm-*` ID from the `x-mnemom-agent` response header |
| `503` | Your personal org is still provisioning (only when `org_id` is omitted) | Safe to retry shortly |
