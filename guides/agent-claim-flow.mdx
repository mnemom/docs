...
4. **Returns** the agent's initial `trust_rating` (computed from the alignment card + the platform default cascade — the platform floor `tp-platform-standard`), plus identity metadata.
...
```json
{
  ...
  "alignment_card_url": "/v1/alignment/agent/ag_..."
}
```
...
The owner remains the principal of record — every subsequent decision the agent makes flows through the owner's org, billing, and policy posture. Revoking the agent ([`DELETE /v1/agents/{id}`](/api-reference/endpoint/delete-agents-agent-id)) is an owner-side operation.
...
- [API reference — overview](/api-reference/overview) — endpoint docs root; see the Postures + Agents groups for the claim flow's HTTP surface (`POST /v1/agents`, `POST /v1/agents/{id}/claim`).
