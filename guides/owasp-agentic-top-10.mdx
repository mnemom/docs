...
<Note>
Tool-result screening runs synchronously, inside the same request that carries the tool result back to the model, before the model is allowed to read it. Requires `screen_surfaces.tool_responses` on the [protection card](/concepts/protection-card).
</Note>
