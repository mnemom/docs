...
| Indirect injection | `indirect_injection` | Front door (turn-internal, once per tool result) |
...
| `indirect_injection` | **ASI01** Agent Goal Hijack | Partial — indirect variant | L1 instruction-delimiter detection; MinHash similarity against known payloads; L2 instruction-structure detection in tool results. Runs turn-internally — each tool result is screened inline before the request is forwarded, and a flagged one is withheld from that same request. Novel payloads with no similarity to known patterns score lower — see [limits](#gaps-and-limits) |
...
**Recommended defense-in-depth:** treat memory stores as an untrusted input boundary, apply the same L1/L2 screening to memory-retrieved content as the gateway applies inline to external tool results, and enable AIP to catch the reasoning anomalies that poisoned memory produces.
...
Indirect injection via tool results is partially covered, and the coverage is **turn-internal**: each tool result is screened inline inside the request that carries it, and in `enforce` a flagged result is withheld — replaced in the outbound request before the model sees it. Nothing about this waits for a later turn or for AIP.

MinHash similarity matching compares tool results against a library of known injection payloads. Sufficiently novel payloads that bear no structural or semantic similarity to known patterns will score below L1 thresholds. L2 runs on the same tool result within the same inline screen — it is a deeper layer of the one screen, not a later re-screen — and detection accuracy is bounded by the analysis model's capability. When both layers miss, the payload lands in that turn's context; AIP then covers the downstream reasoning effect, not the tool result itself.

The arena flywheel closes this gap over time: canary credentials and cross-agent campaign detection surface novel attack infrastructure, and confirmed patterns promote to active detection. For high-sensitivity environments, consider adding application-layer validation of tool results in your own tool-execution path, alongside the gateway's inline screen.
