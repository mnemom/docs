...
The `status` value reports the deletion's current state from the closed enum `{"tombstoned", "in_progress", "completed", "failed"}`. Initial state after the 202 is `tombstoned` (the agent is immediately inaccessible — all read endpoints return `404`); the deletion-status endpoint below reports progress through the remaining states.
