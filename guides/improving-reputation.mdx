---
title: "Improving Your Mnemom Trust Rating"
description: "A practical guide to building, monitoring, and recovering your Mnemom Trust Rating™ — component-by-component strategies for agent builders and operators."
...
Claim your agent on [mnemom.ai/claim](https://mnemom.ai/claim), then authenticate the Mnemom CLI against your account:

```bash
mnemom login
```

Once claimed, your agent has an identity and an [Alignment Card](/concepts/alignment-cards). Confirm the CLI sees it with `mnemom status` (or `mnemom agent list` to list every agent in your account).
</Step>
...
<Tip>
At typical usage, reaching 50 checkpoints takes 1-3 days. Track progress via the reputation endpoint's `checkpoint_count` field, or via the [badge](/guides/reputation-badges):

```bash
curl https://api.mnemom.ai/v1/reputation/{agent_id}
# Look at "checkpoint_count" and "is_eligible" -- is_eligible flips to true at 50
```
</Tip>
</Step>
...
**Review drift alerts promptly.** Check your dashboard or set up [webhook notifications](/guides/webhooks) for `sideband.drift.fired` events (and pair with `drift.resolved` for closeout signals):
...
curl -X POST https://api.mnemom.ai/v1/orgs/{org_id}/webhooks \
  ...
  -d '{
    ...
    "event_types": ["sideband.drift.fired", "drift.resolved"]
  }'
```
...
| Event | Trigger |
|-------|---------|
| `reputation.score_changed` | Score changed by any amount in a recomputation (fires once per real change, not on a no-op recompute) |
...
async function healthCheck(agentId: string) {
  let rep;
  try {
    rep = await getReputation(agentId);
  } catch {
...
The minimum is 50 **analyzed** checkpoints. Checkpoints where the thinking block was below 100 tokens receive a synthetic clear verdict and are not counted toward the 50-checkpoint minimum. Fetch the reputation endpoint and check `checkpoint_accounting`:

```bash
curl https://api.mnemom.ai/v1/reputation/{agent_id}
# checkpoint_accounting.analyzed must be >= 50; checkpoint_accounting.excluded
# breaks out how many were synthetic, below the thinking-token minimum, or quarantined
```
</Accordion>
