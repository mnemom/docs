...
### 2. Violation Recency (20% of score)

**What it measures:** How recently violations occurred. Recent violations penalize more heavily than old ones.

**Formula:** `1000 * (1 - max_recent_impact)` with exponential decay (1-week half-life)

**Time is your friend.** Violations decay automatically -- a `boundary_violation` from 4 weeks ago has only 6.3% of its original impact. But new violations reset the clock.

#### How to Improve

**Prevent new violations.** This is the single most important action. Every new `boundary_violation` resets the recency penalty to maximum. Address the root causes:
...
**Wait for decay.** The exponential decay schedule works in your favor:

| Time Since Last Violation | Recency Score |
|--------------------------|---------------|
| 0 (just happened) | 0 |
| 1 week | 500 |
| 2 weeks | 750 |
...
After 8 weeks without a new violation, the Recency component effectively reaches 1000.
...
5. **Monitor daily.** Check the score trend daily. You should see improvement within 1-2 weeks as the Violation Recency component decays.
...
7. **Address secondary components.** Once Integrity Ratio and Violation Recency are improving, focus on Drift Stability and Trace Completeness.
...
2. **Has enough time passed?** The Violation Recency half-life is 1 week. Meaningful improvement takes 2-4 weeks of clean operation.
...
- A new `boundary_violation` (resets Violation Recency to 0)
