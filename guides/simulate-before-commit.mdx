...
If `candidate_tool_call` is set without `candidate_input`, the platform synthesizes an assistant message carrying the `toolUses` block. If both are set, the explicit `candidate_input.messages` wins for what gets evaluated as conversation context, but tool identifiers are collected from *both*: `candidate_tool_call.tool_name` and any `candidate_input.messages[].toolUses[].name` entries.

<Warning>
  Simulate needs at least one named tool to produce a meaningful verdict. A plain conversational `candidate_input.messages` (user/assistant text with no `toolUses[]` block) names zero tools, and so does an empty or omitted body — either way the response comes back `allowed: "false"` with a `no_tool_named` condition, not a "this spec composes cleanly" green light. Always include a `candidate_tool_call.tool_name` (or a `toolUses[]` entry) for the call you actually want a verdict on.
</Warning>
...
| Tools evaluated | Gateway | Observer | + missing receipts? | `allowed` |
|---|---|---|---|---|
| 0 | * | * | * | `"false"` (`no_tool_named`) |
| > 0 | pass | pass | no | `"true"` |
| > 0 | fail | * | * | `"false"` |
| > 0 | * | fail | * | `"false"` |
| > 0 | warn | * | * | `"conditional"` |
| > 0 | * | warn | * | `"conditional"` |
| > 0 | pass | pass | yes | `"conditional"` |
