...
client_id=<your_client_id>&scope=mcp:read%20mcp:write
```
...
| Access token | 28800 s (8 hours); the outgoing access token runs to its own 8-hour expiry after a refresh — it is not cut short. |
| Refresh token | 30 days sliding; single-use — each exchange issues a new refresh token. The outgoing refresh token has a **60 s grace window** after rotation before it is invalidated, to protect against network races. |
