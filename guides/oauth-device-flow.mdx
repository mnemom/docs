...
<Steps>
  <Step title="Request device and user codes">
    The agent sends `POST /oauth/device_authorization` and receives a `device_code`, `user_code`, `verification_uri`, `expires_in`, and `interval`.
  </Step>
  ...
  <Step title="Agent polls for the access token">
    The agent polls `POST /oauth/token` with `grant_type=urn:ietf:params:oauth:grant-type:device_code` at the prescribed interval until it receives an access token or a terminal error.
  </Step>
</Steps>
...
<!-- TODO: verify endpoint path against mnemom-api/src/oauth/handlers.ts before merging — RFC 8628 default used here -->
...
```json
{
  ...
  "user_code": "WDJB-MJHT",
  "verification_uri": "https://mnemom.ai/activate",
  "verification_uri_complete": "https://mnemom.ai/activate?user_code=WDJB-MJHT",
  "expires_in": 1800,
  "interval": 5
}
```

<!-- TODO: verify expires_in (1800 s) and interval (5 s) against mnemom-api/src/oauth/device-grant.ts:buildDeviceAuthorizationResponse before merging — RFC 8628 defaults used here -->
...
After the device-authorization request succeeds, poll `POST /oauth/token` at `interval`-second intervals. On each `slow_down` response, permanently increase the interval by 5 seconds for the remainder of the session. Stop when you receive an access token or a terminal error.
...
def poll_for_token(device_code: str, interval: int, expires_in: int) -> dict:
    ...
    while time.time() < deadline:
        time.sleep(interval)
        resp = httpx.post(
            "https://api.mnemom.ai/oauth/token",
            data={
...
async function pollForToken(
  ...
  while (Date.now() < deadline) {
    ...
    const resp = await fetch("https://api.mnemom.ai/oauth/token", {
      method: "POST",
...
The following error codes may be returned by `POST /oauth/token` during the poll loop:
...
The refresh token can be exchanged for a new access token via the standard `refresh_token` grant at `POST /oauth/token` before the access token expires.
...
| Item | TTL |
|---|---|
| `device_code` / `user_code` | `expires_in` seconds from the device-authorization response (typically 1800 s / 30 min) |
| Access token | 3600 s (1 hour) |
| Refresh token | Single-use; each exchange issues a new one. |
...
<Note>
If you do not yet have a `client_id`, register your OAuth client with `POST /oauth/register` (RFC 7591 dynamic client registration) before starting the device flow. Public clients (no client secret) are supported. See the [OAuth register endpoint](/api-reference/endpoint/post-oauth-register).
</Note>
