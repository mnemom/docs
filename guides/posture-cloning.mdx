...
Cloning a posture from a different org is **not** supported by default. The clone API requires the caller to be an org admin of the target org **and** at least a member of the source org. Cross-org sharing patterns (template marketplaces, multi-tenant baselines) are debt #5 — track in `safe-house-hardening/debt/post-t1-3-1-cleanup.md`.
