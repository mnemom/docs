...
The endpoint is rate-limited to **1 request per second per IP**, fail-open if the rate-limit store is unavailable. The rate-limit counter has a 60-second minimum window, so the *effective* practical bound is closer to 1 request per minute per IP.
