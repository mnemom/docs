...
When a response contains exactly `limit` rows (more may be available), the bundle carries a
top-level `next_after` field — a Mnemom addition outside the STIX 2.1 bundle spec, which STIX
consumers simply ignore as an unknown field:
...
Pass `next_after` as `?after=` on the next request to paginate forward.
...
def fetch_all_iocs(api_key: str, ioc_type: str | None = None, since: str | None = None):
    ...
    while True:
        ...
        # Look for the bundle-level next_after field to keep paginating.
...
**Recommended polling cadence:** every 5–15 minutes via cron. The IoC publication rate at GA is very low (calm-at-GA contract); more frequent polling burns rate-limit budget without producing more signal. Polling is currently the only integration path — IoC insertions are not delivered as a customer-subscribable webhook event.
...
the published [`/trust/slos`](https://www.mnemom.ai/trust) commitments.
</Note>
