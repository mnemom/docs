...
<Note>
A public status page at `status.mnemom.ai` is planned and not yet live. Until it ships, incident communications go through email (for contractual customers) and the in-app dashboard banner.
</Note>
...
A public status page at `status.mnemom.ai` is planned and will surface:
