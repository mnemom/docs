...
- **The Managed Rule signing chain** — [Managed Rules](/concepts/managed-rules) carry Ed25519 per-row promotion signatures (`promotion_signature`); the primary and secondary storage-tier envelopes are independently signed; the gateway verifies on every read. The wire format reference for downstream evidence systems is at [Managed Rule envelope schema](/specifications/managed-rule-envelope-schema).
