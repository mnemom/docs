...
Update your agent's alignment card directly with a `PATCH` request. If the agent does not yet have a card, one will be created automatically.

Authenticate with either a Bearer JWT or an API key:
...
See the [Unified Cards overview](/api-reference/unified-cards-overview) for the current unified alignment-card endpoints, which supersede the legacy card PATCH surface (UC-4, 2026-04-15).
