...
For protocol-level interop with external agents (A2A, MCP), the same unified shape is also the **AAP protocol-level card** specified at [/concepts/alignment-cards](/concepts/alignment-cards). Production cards you manage here are a superset that adds `conscience`, `capabilities`, `enforcement`, and `integrity_config` on top of that protocol surface.
</Note>
...
`values.declared` accepts any value identifier — validation is shape-only (a non-empty string or `{id, ...params}`), so nothing stops you from inventing one. What actually belongs there, the recommended baseline, and the distinction between a value and a role/capability descriptor are covered in [Alignment Cards § What belongs in values.declared](/concepts/alignment-cards#what-belongs-in-valuesdeclared) — read that before picking your list.

For a value outside that baseline, add a `definitions` entry so it's self-documenting:
...
```bash
mnemom card evaluate my-card.yaml --tools mcp__browser__navigate,mcp__slack__post_message
```
...
- name: Evaluate card policy
  run: npx mnemom card evaluate card.yaml --tools mcp__browser__navigate
  env:
...
Your card composes with your org's alignment template (if one is set) and the platform floor to produce your agent's canonical card — org values and forbidden actions always apply on top of what you declare here. For setting up an org template, granting per-agent exemptions from it, and debugging why a composed field has a value you didn't set, see [Managing Card Composition](/guides/card-composition).

## Validation rules

`mnemom card validate` runs every check below; any failure exits non-zero (there's no separate warning tier — a failed check blocks validation):

| Check | Rule |
|---|---|
| Valid JSON/YAML | Must parse without errors. |
| `card_version`, `agent_id` | Required, non-empty strings. |
| `autonomy_mode`, `integrity_mode` | Required top-level fields, one of `off \| observe \| nudge \| enforce`. |
| `principal` | Required object with `type` and `relationship` from their respective enums; `identifier` required unless `type` is `unspecified`. |
| `values.declared` | Required, non-empty array of strings (or `{id, ...params}` objects). |
| `values.definitions` | If present, every key must match an entry actually in `values.declared` — a definition for an undeclared value fails. |
| `autonomy.bounded_actions` | Required, non-empty array of strings; must be disjoint from `forbidden_actions`. |
| `autonomy.escalation_triggers` | If present, each entry needs a non-empty `condition`, an `action` from a fixed enum, and a non-empty `reason`. |
| `audit` | Required; `retention_days` and `queryable` required, `query_endpoint` required when `queryable` is `true`. |
| Legacy fields | `integrity.enforcement_mode`, `enforcement.mode`, `enforcement.unmapped_tool_action`, `enforcement.fail_open`, and `audit.storage` are all rejected — use the current top-level/unified fields instead. |
| `_composition` | Rejected on an inbound card — it's a server-assigned response field. |

When you're online with an agent to validate against, `mnemom card validate` runs the same check server-side (composed against your org/platform floor) instead of this offline check set — pass `--offline` to force the local rules.
...
When adding new tools, update the card to add both the capability to `bounded_actions` and the tool-to-capability mapping in `capabilities`. The `enforcement` section's `grace_period_hours` field, if you set one, gives you time to make these updates after new tools are first observed before its `default_unmapped_severity` starts applying.
...
- [Managing Card Composition](/guides/card-composition) -- Org templates, exemptions, and debugging composed fields
