...
| Protocol versioning | `AlignmentCard.card_version` | Card schema version enables tooling compatibility |
| Trace format declaration | `audit.trace_format` = `"ap-trace-v1"` | Declares the structured format used |
...
| Obligation | AAP Field | How It Satisfies |
|-----------|-----------|------------------|
| Retention period | `audit.retention_days` >= 90 | Minimum 90 days recommended for EU compliance |
| Queryability | `audit.queryable` = `true` | Traces can be retrieved and inspected |
| Tamper evidence | `audit.tamper_evidence` = `"append_only"` | Audit log integrity protection |
| Query endpoint | `audit.query_endpoint` | Required when `queryable` is true: the API endpoint for trace retrieval |
| Trace format | `audit.trace_format` = `"ap-trace-v1"` | Standardized, versioned format |

**SDK preset**: `EU_COMPLIANCE_AUDIT` provides recommended values:

```python
from aap.compliance import EU_COMPLIANCE_AUDIT
...
|---------------|-------------|-----------|
| Behavioral boundaries | `autonomy.bounded_actions` + `forbidden_actions` | Alignment Card |
| Escalation policy | `autonomy.escalation_triggers` | Alignment Card |
...
Every Integrity Checkpoint carries the agent's identity, linking per-turn analysis back to the agent's declared alignment posture (the AAP Alignment Card).
...
| Concern Category | Article 50 Relevance |
|-----------------|---------------------|
| `prompt_injection` | Content manipulation transparency |
| `value_misalignment` | Declared vs. actual behavior transparency |
| `autonomy_violation` | Human oversight requirements |
| `deceptive_reasoning` | AI system honesty obligations |
| `reasoning_corruption`, `undeclared_intent`, `output_misalignment`, `output_injection_compliance` | Additional integrity-transparency dimensions AIP also detects |

Each detected concern also carries its own severity (`low`, `medium`, `high`, or `critical`), assigned per instance rather than fixed per category. When a concern is detected, the Integrity Checkpoint records:
...
| Per-turn reasoning transparency | AIP | Integrity Checkpoint |
...
| **Article 12** (record-keeping) | Automatic event logs over the system's lifecycle | Append-only audit chain on recipe promotion; signed governance events on the [transparency log](/concepts/transparency-log) |
| **Annex IV** (technical documentation) | Detection mechanism description + accuracy metrics | Signed Managed Rule envelope ([spec](/specifications/managed-rule-envelope-schema)); published availability commitments at [trust.mnemom.ai/slos](https://trust.mnemom.ai/slos); [`/concepts/protection-network`](/concepts/protection-network) for the architectural reference |
