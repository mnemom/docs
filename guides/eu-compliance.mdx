...
| **Article 12** (record-keeping) | Automatic event logs over the system's lifecycle | Append-only audit chain on recipe promotion; signed governance events on the [transparency log](/concepts/transparency-log); [`advisory.published`](/api-reference/webhook-events#advisory-published) webhook |
| **Annex IV** (technical documentation) | Detection mechanism description + accuracy metrics | Signed Managed Rule envelope ([spec](/specifications/managed-rule-envelope-schema)); published [`/trust/slos`](https://www.mnemom.ai/trust/slos) (propagation P95 ≤ 30s, freshness P99 ≤ 5min, failover 99.99%); [`/concepts/protection-network`](/concepts/protection-network) for the architectural reference |
