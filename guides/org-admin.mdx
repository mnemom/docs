...
The `viewer` and `auditor` roles require the **RBAC** capability on your account. If it isn't enabled, assigning either role fails with a message pointing at [Pricing](/pricing/overview).
...
Configure SAML SSO with your IdP (Okta, Azure AD, Google Workspace, etc.).

- [`GET /v1/orgs/{org_id}/sso`](/api-reference/endpoint/get-orgs-org-id-sso) — read the current config (owner or admin)
- [`PUT /v1/orgs/{org_id}/sso`](/api-reference/endpoint/put-orgs-org-id-sso) — configure (owner-only — misconfiguration can lock everyone out of the org)
- [`POST /v1/orgs/{org_id}/sso/test`](/api-reference/endpoint/post-orgs-org-id-sso-test) — test before saving (owner or admin)
- [`DELETE /v1/orgs/{org_id}/sso`](/api-reference/endpoint/delete-orgs-org-id-sso) — remove (owner-only)
...
| Apply a credit, issue a credit note | Email support@mnemom.ai |
...
A newly created org does **not** inherit any invite-gated product access (e.g. Mnemom Agent) you hold personally — that access is granted per account, and a brand-new shared org starts with none. Check `GET /v1/code/entitlement?org_id={org_id}` before relying on it there; if it's not enabled, ask your Mnemom contact for an org-scoped invitation.
...
**Hand off ownership when an admin leaves.** Not self-service — [contact support](mailto:support@mnemom.ai).
