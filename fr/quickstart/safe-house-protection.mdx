...
À partir de là, une requête qui renvoie des résultats d'outils au modèle est filtrée **à nouveau à l'intérieur de cette même requête** — une fois par résultat d'outil, sur sa propre surface, avant que le corps ne soit transmis au fournisseur. C'est une vérification de porte d'entrée ; rien n'attend le tour suivant de l'agent. La couverture n'est toutefois pas inconditionnelle : une requête qui se ramifie vers de nombreux appels d'outils au sein d'un même tour n'a pas de couverture complète garantie.
