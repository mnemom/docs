...
Commencez par le mode `observe`. Il exécute une analyse complète des menaces sans aucun impact sur la latence, vous permettant de voir ce que Safe House détecterait avant de vous engager dans le blocage. La configuration de Safe House se trouve sur la **carte de protection** de l'agent — `mode` est l'interrupteur principal de premier niveau ; `screen_surfaces` décide quelles surfaces le pipeline de détection inspecte, et chaque surface a son propre moment : `incoming` est analysée à l'arrivée de la requête, `tool_responses` à l'intérieur même du tour (une analyse par résultat d'outil, en ligne, avant que la requête ne soit transmise au fournisseur) et `outgoing` à la porte de sortie.

La carte ci-dessous laisse `tool_responses` désactivé pour simplifier le premier essai. L'activer ajoute une analyse attendue par résultat d'outil sur le chemin de la requête et, en `enforce`, permet à Safe House de retenir un seul résultat d'outil en cours de tour — en remplaçant son contenu par un avis de mise en quarantaine pendant que le reste du tour se poursuit — au lieu de rejeter toute la requête. Voir `/concepts/safe-house#turn-internal-tool-result-screening`.
...
`front` est un agrégat de toutes les analyses de la porte d'entrée pour cette requête, pas une évaluation unique : une fois `tool_responses` activé, un tour produit une analyse du message plus une par résultat d'outil, toutes repliées dans cette seule valeur `front=`.
</Note>
