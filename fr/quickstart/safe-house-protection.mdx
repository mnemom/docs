...
À partir de là, une requête qui renvoie des résultats d'outils au modèle est filtrée **à nouveau à l'intérieur de cette même requête** — une fois par résultat d'outil, sur sa propre surface, avant que le corps ne soit transmis au fournisseur. En modes `nudge` et `enforce`, la passerelle prend les trois premiers résultats d'outils que le corps transporte. C'est une vérification de porte d'entrée ; rien n'attend le tour suivant de l'agent.
