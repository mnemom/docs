...
- Vérifications d'[intégrité AIP](/concepts/integrity-checkpoints) en temps réel
...
<Steps>
  ...
  <Step title="Ce qu'il faut lire au retour">
    ...
    Consultez la [référence des en-têtes](/api-reference/headers) pour l'ensemble canonique complet + les parseurs par langage, et la [référence des erreurs](/api-reference/errors) pour le mapping verdict-vers-statut (quand `enforced` devient une quarantaine 422 ou un blocage 403).
  </Step>
...
- [Auto-héberger](/quickstart/self-hosted) si vous avez besoin d'un contrôle complet de résidence des données
