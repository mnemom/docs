...
<Steps>
  ...
  <Step title="Configurer l'environnement">
    ...
    <Warning>
    `SUPABASE_URL` doit être un point de terminaison compatible REST — un projet Supabase hébergé, ou une stack auto-hébergée qui expose l'API PostgREST de Supabase — pas une chaîne de connexion Postgres brute. La passerelle lui parle en HTTP (`GET <SUPABASE_URL>/rest/v1/...`), donc la pointer vers un simple hôte `postgres://` fera échouer chaque appel de données, ainsi que la vérification Supabase de `/health/ready`. Le conteneur `postgres` fourni dans ce fichier compose fournit le stockage devant lequel placer votre propre déploiement Supabase/PostgREST — ce n'est pas lui-même un point de terminaison REST.
    </Warning>
  ...
  <Step title="Vérifier la santé">
    ...
    `status` vaut `"degraded"` (HTTP 503) et la vérification individuelle concernée est `{ "ok": false }` dès que Redis, `SUPABASE_URL` ou la licence ne peuvent pas être atteints/validés — le cas le plus fréquent étant l'échec de `supabase` parce que `SUPABASE_URL` n'est pas encore un point de terminaison compatible REST (voir l'avertissement ci-dessus).
  </Step>
...
<Steps>
  ...
  <Step title="Créer un Secret Kubernetes">
    ...
    Ce sont les clés attendues par le `values.yaml` par défaut du chart (`secrets.values`) ; n'en ajoutez d'autres que si votre propre override de values les ajoute.
  </Step>
...
Le processus de la passerelle ne valide lui-même que ces trois-là — tout autre élément manquant le fait échouer fermé au démarrage avec une `EnvValidationError` nommant la ou les variables manquantes :

| Variable | Description |
|---|---|
| `SUPABASE_URL` | Un point de terminaison compatible REST — projet Supabase hébergé (`https://<ref>.supabase.co`) ou stack auto-hébergée exposant PostgREST. Pas une chaîne de connexion Postgres brute. |
| `SUPABASE_SECRET_KEY` | Clé de rôle de service Supabase |
| `ANTHROPIC_API_KEY` | Clé API Anthropic (requise pour l'analyse AIP) |

`REDIS_PASSWORD` et `POSTGRES_PASSWORD` sont requis à une autre couche — le fichier Docker Compose lui-même refuse de démarrer ces conteneurs sans eux (voir la stack compose ci-dessus). `MNEMOM_LICENSE_JWT` est votre licence Enterprise depuis [mnemom.ai/dashboard](https://mnemom.ai/dashboard) ; la passerelle démarre quand même sans elle, mais la vérification `license` de `/health/ready` la signale comme un déploiement non licencié. `SUPABASE_JWT_SECRET` et `INTERNAL_API_KEY` ne sont pas lus par les rôles gateway/observer que fait tourner cette stack compose — laissez-les non définis sauf si vous auto-hébergez aussi un rôle qui documente en avoir besoin.
...
| `/health/ready` | Sonde de readiness | Vérifie Redis, le point de terminaison Supabase/PostgREST et la validité de la licence |
...
<Warning>
`/metrics` est fermé par défaut — définissez `METRICS_SCRAPE_KEY`, sinon le point de terminaison répond à chaque scrape comme si le chemin n'existait pas (pas de 401, juste un corps générique de type not-found, si bien qu'un scraper mal configuré n'apparaîtra même pas comme une erreur de permission). Présentez-la comme `X-Metrics-Key: <key>` ou `Authorization: Bearer <key>` ; c'est cette dernière forme qui permet à un `ServiceMonitor` Prometheus Operator de l'attacher via `bearerTokenSecret`.
</Warning>
