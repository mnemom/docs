...
Exécutez la Mnemom Gateway sur votre propre infrastructure pour un contrôle complet de la résidence des données. Toutes les traces, points de contrôle d'intégrité et données d'agent restent au sein de votre réseau. La passerelle auto-hébergée est un adaptateur Node.js qui exécute le même code que le service géré Cloudflare Workers — comportement identique, votre infrastructure.
...
<Steps>
  ...
  <Step title="Démarrer la stack">
    ...
    Cela démarre cinq services dans l'ordre :
    1. **PostgreSQL** — base de données avec vérification de santé
    2. **Redis** — couche de cache avec persistance
    3. **Migrate** — applique le schéma de base de données (s'exécute une fois, puis se termine)
    4. **Gateway** — proxy HTTP sur le port 8787
    5. **Observer** — planificateur en arrière-plan pour le traitement des traces
  </Step>
...
|---|---|
| `SUPABASE_URL` | URL du projet Supabase ou point de terminaison REST PostgreSQL |
| `SUPABASE_KEY` | Clé de rôle de service Supabase |
...
Les migrations s'exécutent automatiquement via le service `migrate`.
