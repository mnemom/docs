...
description: "Canonical wire format for AAP attestation tokens (v1) — JWS Compact serialization, EdDSA-signed payload."
sidebarTitle: "Attestation Token"
icon: "file-shield"
---

The canonical wire format for [AAP attestation tokens](/concepts/aap-attestation).
...
| `smolt_id` | string | no | Optional convenience claim for consumers binding on smolt-id. Not itself format-validated by the verifier — smolt IDs conventionally look like `smolt-<hex>`. |
...
Issued and verified server-side by the mnemom-api attestation service (signer + verifier), and independently re-verifiable offline via [`mnemom-platform/cli/src/commands/verify-card.ts`](https://github.com/mnemom/mnemom-platform/blob/main/cli/src/commands/verify-card.ts) (the `mnemom verify-card` CLI command).
