...
Every prerendered URL on a compliant site exposes a sibling preview manifest, advertised via `<link rel="alternate">` in `<head>`. Crawlers and humans ignore the unknown MIME; aware agents follow the link — they do not need to construct the sibling path themselves, but the general convention is: a URL ending in `/` gets an `index.preview.json` appended (`https://example.com/methodology/` → `https://example.com/methodology/index.preview.json`); a URL not ending in `/` gets a `.preview.json` suffix instead (`https://example.com/methodology` → `https://example.com/methodology.preview.json`). The site root follows the trailing-slash rule (`/` → `/index.preview.json`).
...
1. **Generate a preview-manifest sibling at build time** for every public URL, following the path convention in [How discovery works](#how-discovery-works) above. Walk your prerendered HTML, extract title / description / canonical / og:image, classify the type, write the manifest.
...
4. **Write a verifier** that validates your manifests against the v1 schema and runs in CI.
...
5. **Cross-reference from your `/agents.txt`**:
   ...
   agent-preview-discovery: <link rel="alternate" type="application/vnd.agent-preview+json" href="<url>index.preview.json">  # <url> is the page's trailing-slash URL, e.g. /methodology/index.preview.json
   agent-preview-example: https://yoursite.com/methodology/index.preview.json
   ```
...
Mnemom's own implementation covers the full round-trip: a build-time generator that writes each route's manifest, a discovery emitter that adds the `<link rel="alternate">` tag, and a CI verifier that checks every manifest against this schema before deploy.
