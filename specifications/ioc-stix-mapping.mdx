...
When the response carries exactly `limit` rows (i.e. there may be further rows available), the bundle includes a plain top-level `next_after` field — not wrapped in a STIX `extensions` block — whose value is the ISO-8601 `last_seen_at` timestamp of the final indicator in the bundle. Resume by passing that value as `?after=` on the next request. STIX consumers that do not recognize the field ignore it (it is outside the `bundle` object's defined STIX properties).
...
| `after` | ISO-8601 string | (none) | Pagination cursor: returns rows with `last_seen_at` strictly before this value. |
...
**Auth:** none. This is a public, unauthenticated endpoint — no API key required.

**Rate limit:** nominally 1 request per second per IP (best-effort; fails open if the rate-limit store is unavailable), but the rate-limit counter has a 60 s minimum window, so the *effective* practical bound is 1 request per minute per IP. Recommended polling cadence: every 5-15 minutes via cron. See the [IoC feed consumption guide](/guides/ioc-feed-consumption) for a runnable integration.
...
Customers wanting cryptographic attestation for cross-tenant detection content should rely on the append-only audit chain (`recipe_review_actions`) plus the published [`/trust/slos`](https://www.mnemom.ai/trust/slos) commitments. (`recipe.promoted` and `advisory.published` are Mnemom-internal operations signals, not customer-subscribable webhooks — see the note on [Webhook Event Catalog](/api-reference/webhook-events#detection-reports).)
</Note>
