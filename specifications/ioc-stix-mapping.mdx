...
**Rate limit:** nominally 1 request per second per IP (best-effort, KV-backed; fails open if the rate-limit KV namespace is unbound), but Cloudflare Workers KV has a 60 s minimum TTL, so the *effective* practical bound is 1 request per minute per IP. Recommended polling cadence: every 5-15 minutes via cron. See the [IoC feed consumption guide](/guides/ioc-feed-consumption) for a runnable integration.
