...
|---|---|---|---|
| **KV envelope** | Workers KV key `sh:aegis-recipes:v1` | `RECIPE_KV_SIGNING_KEY` | Primary read path; no expiration — the promoter overwrites the key on every promotion, so staleness is governed by `signed_at`, not KV TTL |
...
Internal detection-routing fields (the XFD canonical fields `door`, `threat_type`, `variant_class`, `evasion_technique`, `mitre_atlas`, and the detection content itself) are out of scope for this spec — the row carries them on the wire, but the platform writes/reads them internally; this page documents what a gateway or tooling integrator needs, not the full internal shape.
...
2. **Workers KV primary** — no expiration (overwritten on every promotion); signed with `RECIPE_KV_SIGNING_KEY`.
...
<Note>
The production R2 binding uses its own environment-suffixed bucket (`aegis-recipes-failover-prod`), separate from staging's, so a production read can never cross-read a staging envelope.
</Note>

## 7. Failover alert tags

The gateway emits stable alert tags on the failover path. These are operator-facing diagnostic tags drawn from the cell-matrix vocabulary; customers may see the corresponding SLO impact on [`/trust/slos`](https://www.mnemom.ai/trust/slos) but do not consume these tag values directly.
