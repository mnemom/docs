...
<Tip>
Use the verification endpoint to independently confirm that a reputation score is backed by real integrity data. The `latest_certificate_hash` can be cross-referenced with the [certificate endpoint](/api-reference/endpoint/get-checkpoints-id-certificate) and members of the agent's org can check the `merkle_root` against the [Merkle root endpoint](/api-reference/endpoint/get-agents-id-merkle-root) (authenticated).
</Tip>
