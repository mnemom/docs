...
| `safe_house_blocked` | 403 | The agent's `protection.mode = enforce` blocked the inbound message. Emitted for the `incoming` surface only — a blocked tool result is withheld from the forwarded request instead of failing it. The response body's `details` includes the verdict and (when available) the quarantine ID. See [Safe House verdict-to-status mapping](#safe-house-verdict-to-status-mapping) below. |
...
A `422 Unprocessable Entity` is the dedicated status code for Safe House `quarantine` verdicts **on the `incoming` surface**: the inbound message was structurally valid (so not 400) and you were authorized (so not 403), but the content tripped the quarantine threshold and the gateway is holding it for review instead of forwarding it to the upstream provider. A quarantined *tool result* is handled per surface instead — withheld from the forwarded request, 200 returned — so it never reaches this status code.
...
The Safe House front-door screens the inbound message when the request arrives and, when `screen_surfaces.tool_responses` is enabled, each tool result the request carries — inline, in the same turn. It therefore produces a **set** of per-surface decisions per request, which `X-Mnemom-Verdict.front` rolls up into a single value (`pass | observed | nudged | enforced`). When the agent's `protection.mode` is `enforce`, only the **`incoming`-surface** decision can drive the HTTP response:
...
**Tool-result decisions do not map to a status code.** A `block` or `quarantine` on the `tool_responses` surface is a per-surface *withhold*, not a request rejection: the gateway replaces that tool result's content in the outbound request with a quarantine notice and forwards the turn anyway. The response is a normal 200 with `front=enforced` on the verdict header, and there is no `safe_house_blocked` / `safe_house_quarantined` error body. Do not treat "no 4xx" as "nothing was intercepted" — see [Turn-internal tool-result screening](/concepts/safe-house#turn-internal-tool-result-screening).
