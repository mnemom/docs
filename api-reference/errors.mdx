...
The Safe House front-door inspects every inbound surface a request carries — the inbound message, and each tool result the request is handing back to the model — and emits a verdict (`pass | observed | nudged | enforced`). When the agent's `protection.mode` is `enforce`, the verdict drives the HTTP response:
...
<Note>
**A front-door finding on a tool result does not produce a 4xx.** The table above describes the front door acting on the *inbound message*. When the front door acts on a **tool result** carried in the same request — which it does synchronously, before the body is forwarded upstream — the request is not failed. The offending tool result is withheld (its content replaced with a quarantine notice) or decorated as untrusted, the rest of the body is forwarded, and you get a **200 with `front=enforced`** on `X-Mnemom-Verdict`. Do not read a 200 as "no front-door action this turn": read the header.
</Note>
