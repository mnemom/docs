...
Quarantined items are held pending human review — an inbound message that was stopped at request arrival, or an individual tool result that was withheld from the request carrying it. Reviewers can release (with or without a false-positive flag) or confirm as a genuine threat.
...
| `POST` | `/v1/safe-house/quarantine/:id/release` | Release the quarantined item to the agent; optionally mark as false positive |
| `POST` | `/v1/safe-house/quarantine/:id/report` | Confirm the quarantined item as a genuine threat |
...
Returns a structured compliance report covering all evaluation decisions, blocked/quarantined content, false positive resolutions, and configuration change audit records within the requested window. Supports `Accept: text/csv` for spreadsheet-compatible export.

Decisions are reported **per screened surface, not per turn**: the front door screens the inbound message when the request arrives and each tool result [turn-internally](/concepts/safe-house#turn-internal-tool-result-screening), so one turn can contribute several rows. A `tool_responses` row records a *withheld* tool result — the offending content was replaced in the forwarded request and the turn continued — rather than a rejected request, so row counts will not reconcile against 4xx response counts.
