...
```json
{
  ...
  "coherence": { "...": "dimensional coherence vector — governance median, conflict-edge count, conscience/integrity invariants" },
  "fault_lines": [
    {
      "id": "fl-001",
...
| `coherence` | object | Dimensional team-coherence vector (governance median, conflict edges, conscience/integrity invariants). Replaces the deprecated scalar `fleet_score` |
...
| `404` | Agent not found |
| `409` | Conflicting concurrent transaction |
...
Every intelligence endpoint can return `400` (invalid request), `401` (missing/invalid API
key), `404` (resource not found), `429` (rate limited — check `Retry-After`), or `500` (server
error, retry with backoff). `POST /teams/recommend-policy` and `POST /transactions` can also
return `409` on a conflicting concurrent request.
