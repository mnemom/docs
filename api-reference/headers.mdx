...
|---|---|---|---|---|---|
| **`front`** (Protection inbound) | Every front-door screen on this request ran clean. | Threat observed but not acted on (`mode=observe`). | Inbound content carried a guidance nudge (`mode=nudge`). | Inbound content was replaced or quarantined same-turn (`mode=enforce`) — the message itself, or any one of the request's tool results. | Not produced by this checkpoint — Safe House evaluation fails closed internally rather than surfacing an unverified state. |
...
The four-checkpoint model is documented in the [Safe House concept](/concepts/safe-house). All four checkpoints **always run** on every gateway-routed request — `pass` means every screen that checkpoint performed ran and came back clean. A checkpoint is a *position* in the request lifecycle, not a single evaluation: the front door screens each enabled surface separately, so it commonly fires more than once inside one request.

<Note>
**`front` is a per-request rollup, not a count of screens.** The front door runs more than once per turn: the inbound message is screened when the request arrives, and — when `screen_surfaces.tool_responses` is enabled — each tool result the request carries is screened turn-internally before the request is forwarded. One `front=enforced` can therefore roll up several per-surface decisions: a rejected inbound message, one or more withheld tool results, or both. Conversely, `front=pass` means *every* screen on that request came back clean — not that only one ran. Per-surface detail lives in the `X-Mnemom-Advisory` entries and in `sh_evaluations`; see [Turn-internal tool-result screening](/concepts/safe-house#turn-internal-tool-result-screening).
</Note>
