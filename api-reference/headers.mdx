...
| `X-Mnemom-Schema` | Always (mnemom-api) | mnemom-api | Response-shape identifier — the unified card schema (`unified/2026-04-15`) by default, or a more specific value on a per-endpoint response (e.g. `alignment-card/sources`, `safe-house-harness-state/v1`). Mirrors the shape your client should validate against. |
...
|---|---|---|
| `X-AIP-Verdict` | When AIP analysis ran | Raw AAP verdict for SDK clients (`clear \| review_needed \| boundary_violation \| skipped \| pending \| error`). The structured `X-Mnemom-Verdict.integrity` is the canonical rollup; `X-AIP-Verdict` is the raw value SDK clients use to fetch the full attestation via `Checkpoint-Id`. |
...
|---|---|---|---|---|---|
| **`front`** (Protection inbound — message *and* tool results) | No threat detected on any inbound surface. | Threat observed but not acted on (`mode=observe`). | Inbound content carried a guidance nudge (`mode=nudge`). | Inbound content was replaced or quarantined same-turn (`mode=enforce`) — this includes a tool result withheld or decorated in the request that carried it. | Not produced by this checkpoint — Safe House evaluation fails closed internally rather than surfacing an unverified state. |
...
<Note>
**`front` is a rollup, not a single screen.** There is one `front` value per response, but the front door does not run once per turn. It runs once per inbound surface the request carries — the message, and each tool result travelling back to the model in that same request body — and `front` reports the strongest outcome across all of them. Tool results have no separate checkpoint field precisely because they are screened at the front door, synchronously, before the body is forwarded upstream. `autonomy` covers tool *calls* going out; tool *results* coming back are `front`. See [When the front door runs](/concepts/safe-house#when-the-front-door-runs).
</Note>

<Note>
Verdict timing is mode-dependent: in `enforce` the `integrity`/`back` verdicts are resolved before the response is returned (streaming adds latency to preserve this); in `observe`/`nudge` analysis is post-hoc, so a backgrounded checkpoint's same-turn header value is provisional. The `front` checkpoint is not backgrounded in `enforce` — every inbound surface it screens, tool results included, is resolved and acted on before the request is forwarded upstream. `off` runs no analysis.
...
UUIDv4 per request. Stable across the entire request lifecycle. When you open a support ticket, paste this header value — we can pull every log line, OTel span, and audit row for the request.

```http
X-Mnemom-Request-Id: 8f446ed6-ca87-4c1d-aa90-e2bc6e9ef580
```

The header is **always emitted on gateway-routed responses** (the LLM-proxy surface at `gateway.mnemom.ai` — `/anthropic/*`, `/openai/*`, `/gemini/*`), including 4xx and 5xx error responses. Even on auth-failure 401s, the `X-Mnemom-Request-Id` is set — paste-into-support works on the failure case too.

<Note>
**REST calls to `api.mnemom.ai` carry `X-Request-Id`, not `X-Mnemom-Request-Id`.** The two APIs are deliberately kept separate: mnemom-api's own response allow-list does not include `X-Mnemom-Request-Id` (it is reserved as gateway-owned), so every `/v1/*` REST response — success or error — instead carries a plain `X-Request-Id`, generated fresh unless your client already sent one. It serves the same support-correlation purpose: paste it into a ticket for that request's server-side logs. See [Errors](/api-reference/errors) for how this shows up on REST error responses.
</Note>
...
## Other `api.mnemom.ai` (REST) response headers

Outside the gateway's canonical set above, plain REST calls to `api.mnemom.ai` also carry a few operational headers that are not `X-Mnemom-`-namespaced verdict/advisory state:

| Header | Always emitted? | Purpose |
|---|---|---|
| `X-Request-Id` | Always | Per-request correlation id for REST calls — see the note above. |
| `X-Mnemom-Min-CLI` | Always | The minimum `@mnemom/mnemom` CLI version the server accepts; the CLI reads this and exits with an upgrade prompt if it's below the floor. |
| `Deprecation` / `Sunset` / `Link` | Only when your pinned `X-Mnemom-Version` is deprecated | RFC 8594 deprecation signaling — `Sunset` carries the retirement date, `Link` points at the versioning guide. |
