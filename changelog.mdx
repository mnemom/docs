...
## Mnemom Trust Score™ (E-06)
<sub>February 2026</sub>

**The credit score for AI agents.** The Mnemom Trust Score™ is a composite trust metric (0--1000) computed from independently verified behavioral data -- integrity checkpoints, drift stability, trace completeness, and fleet coherence. No self-reporting. No benchmarks. Just verified behavior.

- **Composite Score**: Weighted formula across five components: Integrity Ratio (40%), Violation Recency (20%), Drift Stability (20%), Trace Completeness (10%), Coherence Compatibility (10%)
- **Bond-Rating Grades**: AAA through CCC letter grades with tier labels (Exemplary, Established, Reliable, Developing, Emerging, Concerning, Critical)
- **Confidence Levels**: Four tiers (insufficient, low, medium, high) based on checkpoint count, with a minimum of 50 analyzed checkpoints for score publication
- **Anti-Gaming**: Minimum checkpoint thresholds, synthetic checkpoint detection, rate limiting, independent verification by a separate analysis LLM
- **Embeddable Badges**: Dynamic SVG badges in 4 variants (score, score+tier, score+trend, compact) with embed code for Markdown, HTML, React, and A2A Agent Cards
- **A2A Trust Block**: `trust` extension for A2A Agent Cards with `ReputationGate` middleware for programmatic trust thresholds
- **Full API**: Public score retrieval, weekly history snapshots, badge SVG endpoint, batch lookup (50 agents), directory search, side-by-side comparison (2--10 agents), and ecosystem benchmarks
- **Webhook Events**: `reputation.score_changed` and `reputation.grade_changed` for real-time monitoring
- **SDK**: `fetchReputation()`, `fetchReputationHistory()`, `searchDirectory()`, `getA2AReputationExtension()` in TypeScript and Python
- **[Reputation Scores](/concepts/reputation-scores)** | **[Scoring Methodology](/protocols/aap/reputation-methodology)** | **[Improving Reputation](/guides/improving-reputation)** | **[Badges](/guides/reputation-badges)** | **[API Reference](/api-reference/reputation-overview)**
