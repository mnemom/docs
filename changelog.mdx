...
- Two opt-in toggles: **implicit goal mode** infers a goal contract from your own messages
  instead of requiring one up front, and `--no-context` turns off the gateway's
  context-folding summarization for a session that needs the full, unsummarized window.
